...
//Descripción:      Página principal: Conductores
//Actualizaciones
...
@section('content')
<!-- MENU ADMINISTRATIVOS: CONDUCTORES -->
@include('partials.menu-conductores')
	<div class="container text-center">
		...
		<div class="row text-center">
			<div class="col-md-5">
				<!-- BOTON REGRESAR -->
		        <a href="{{ route('admin-conductores') }}" class="btn btn-primary boton btn-lg" data-toggle="tooltip" title="Regresar...">
		        	<span class="fa fa-mail-reply" style="font-size:18px;text-shadow:2px 2px 4px #000000;">
		        		<font face="verdana" color="White" style="font: arial; font-size:14px;text-shadow:2px 2px 4px #000000;">&nbsp&nbspCONDUCTORES</font>
		        	</span>
		        </a>&nbsp&nbsp
	          	<!-- REGISTROS - NUEVO -->
					...
					<tr>
						<td>
							...
							@if(Auth::user()->m_capreed=="SI")
								<a onclick="consultar({{$propietario->m_nucodig}}, '{{$propietario->m_nucociu}}')" id="btneditar" class="btn btn-primary" data-toggle="modal" data-target="#modaleditar" title="Editar registro" data-placement="right"><i class="fa fa-edit" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>
								</a>
							@else
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-check"></span>&nbsp&nbspCREAR CONDUCTOR
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					...
					{!!Form::open(['route'=>'conductores.store','class'=>'form-horizontal']) !!}
					@include('admin.conductores.conductores.form')
				</div>
				<!-- Pie -->
		...
		<div id="modaleditar" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-header" style="background-color: #3498db">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-edit"></span>&nbsp&nbspEDITANDO CONDUCTOR
					...
					<div class="modal-body">
						{!!Form::open(['id'=>'formeditar','class'=>'form-horizontal']) !!}
							...
							@if (\Session::has('men_reg_edi'))
								...
								@include('partials.mensajes')
							@endif
							@include('admin.conductores.conductores.formb')
							<!-- Pie -->
							<div class="modal-footer">
								<div class="text-center">
									<!-- BOTON CANCELAR -->
	...
	<!-- INICIO FUNCION: ELIMINAR data[0].m_canombr -->   
	<script>
		function eliminar() {
	...
	<script>
	    function consultar(id, idCiudad){
	    	...
	    	$.ajax({
                url: "conductores/"+id+"/edit",
                type: "get",
                success:function(data){                	
                	$("#modalcrear").attr('disabled','disabled');
                	$("#modalcrear").children().attr("disabled","disabled");
            
                    $("#formeditar").attr("action","conductores/"+id+"");
                    $('#modaleditar #m_id').val(data[0].m_nucodig);
                    $('#modaleditar #m_canuipe').val(data[0].m_canuipe);
                    $('#modaleditar #m_nudiver').val(data[0].m_nudiver);
                    $('#modaleditar #m_nutipdo').val(data[0].m_nutipdo);
                    $('#modaleditar #m_canombr').val(data[0].m_canombr);
                    $('#modaleditar #m_caprnom').val(data[0].m_caprnom);
                    ...
                    $('#modaleditar #m_caplaca').val(data[0].m_cavehac);
                    //$('#modaleditar #m_canuint').val(data[8][0].m_canuint);

                    $('#tituloeditar').html(data[0].m_canombr);
                    $('#tituloeliminar').html(data[0].m_canuipe + ' - ' + data[0].m_canombr);
                    $("#formeliminar").attr("action","conductores/"+id+"");
					...
					for(var k in data[2]) {
						$('#modaleditar #tablaSegurosPersonas tbody')
						    .append($('<tr>')
								.append($('<td>').append($('<button>')
									             .attr('id',data[2][k].m_nucodig+'editartablaSegurosPersonas')
									             .attr('type','button')
									             .attr('class','btn btn-primary')
									             .addClass('fa fa-edit')
									             .attr('style','font-size:18px;text-shadow:2px 2px 4px #000000;')
		...
		function editartablaDocumentosPersonas(val) {
			...
			$(".editartablaDocumentosPersonas").show()
			...
			$('#modaleditar #m_causreg_d').val($('#modaleditar #' + idButton).data('usuregdocper')[0].name);
			$('#modaleditar #m_causact_d').val($('#modaleditar #' + idButton).data('usuactdocper')[0].name);
		...
		function editarTablaSegurosPersonas(val) {
  			var dataResult = JSON.parse(val.dataset.myval)
  			var idButton = $(val).attr('id')
  			$(".editartablaSegurosPersonas").show()
  			...
  			//$('#modaleditar #m_causreg_s').val($('#modaleditar #' + idButton).data('usuregsegper')[0].name);
  			//	DATOS FECHA Y USUARIO ACTUALIZA
  			$('#modaleditar #updated_at_s').val(dataResult.updated_at);
  			//$('#modaleditar #m_causact_s').val(dataResult.m_causact);
  			//$('#modaleditar #m_causact_s').val($('#modaleditar #' + idButton).data('usuactsegper')[0].name);
	...
	<div id="modalprocedimientos" class="modal fade">
	    ...
	    <div class="modal-content">
			...
			<div class="modal-header" style="background-color: #3498db">
				...
				<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-list-alt"></span>&nbsp&nbspPROCEDIMIENTOS CONDUCTOR
				</h3>
			</div>
			<!-- Contenido -->
			<div class="modal-body">
		    	<div class="container-fluid">
					<object width="100%" height="50px" data="{{ asset('document/conductor.pdf') }}"></object>
		    	</div>
		    </div>
	    	<!-- Pie -->
			<div class="modal-footer">
		...
		<div class="modal-dialog modal-lg">
		    <div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-filter"></span>&nbsp&nbspFILTROS CONDUCTOR
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					{!!Form::open(['route'=>'conductores.index','method'=>'get'])!!}
			    	<div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-6 text-right" style="padding-right: 0px">
						...
						@if(Auth::user()->m_capreep=="SI")
							<form action="{{ route('conductores-pdf') }}" method="get">
								<!-- ORDENAMIENTO -->
					...
					<div class="col-md-6 text-left" style="padding-left: 8px">
						...
						@if(Auth::user()->m_capreex=="SI")
							<form action="{{ route('conductores-excel') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))

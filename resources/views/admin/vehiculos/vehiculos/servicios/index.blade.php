...
//Descripción:      Página principal: Servicios vehiculos
//Actualizaciones
...
@extends('template')
@section('title','Servicios vehículos')
@include('sweet::alert')
...
@include('partials.menu-vehiculos')
	<div class="container text-center">
		<!-- ADMINISTRATIVOS - SERVICIOS VEHICULOS -->
		<hr class="separador">
				...
				<form action="{{ route('servicios.index') }}" method="get">
					...
					<div class="col-md-5 text-left" style="padding-left: 0px">
						<button 
							...
							</i>&nbsp&nbsp&nbsp(&nbsp&nbspTotal Registros {{count($servicios)}} de {{$servicios->total()}}&nbsp&nbsp)
						</button>
					</div>
				</form>
			</div>
			<!-- PAGINADO SUPERIOR -->
			@if($servicios->total()>160)
					...
					<tr>
						<td>
							...
							@if(Auth::user()->m_capreed=="SI")
								<a onclick="consultar({{$servicio->m_nucodig}})" id="btneditar" class="btn btn-primary" data-toggle="modal" data-target="#modaleditar" title="Editar registro" data-placement="right"><i class="fa fa-edit" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>
								</a>
							@else
						...
						<td class="text-left">{{$servicio->m_canombr}}</td>
						<td>{{$servicio->m_cacodna}}</td>
						<td>{{$servicio->m_caestad}}</td>
						<td>{{$servicio->created_at}}</td>
						<td>{{$servicio->usuario_registra->name}}</td>
						<td>{{$servicio->updated_at}}</td>
						<td>{{$servicio->usuario_actualiza->name}}</td>
					</tr>
				@endforeach
			</tbody>
			<!-- FIN CONTENIDO -->
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-check"></span>&nbsp&nbspCREAR SERVICIO VEHICULO
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					...
					@include('admin.vehiculos.vehiculos.servicios.form')
				</div>
				<!-- Pie -->
		...
		<div id="modaleditar" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-header" style="background-color: #3498db">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-edit"></span>&nbsp&nbspEDITANDO SERVICIO VEHICULO
							...
							<div class="modal-footer">
								<div class="text-center">
									...
									@if(Auth::user()->m_capreel=="SI")
										<a onclick="eliminar({{$servicio->m_nucodig}})" id="btn btn-danger" class="btn btn-danger" data-toggle="modal" data-target="#modaleditar" title="" data-placement="right"><i class="fa fa-trash" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>&nbsp&nbspEliminar
										</a>
									@else
	    ...
	    function consultar(id){
	    	...
	    	$.ajax({
                ...
                success:function(data){
                    ...
                    console.log(data);
                    $("#formeditar").attr("action","servicios/"+id+"");
                    ...
                    $("#formeliminar").attr("action","servicios/"+id+"");
                }               
            })     
	    }
	...
	<div id="modalprocedimientos" class="modal fade">
	    ...
	    <div class="modal-content">
			...
			<div class="modal-header" style="background-color: #3498db">
				...
				<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-list-alt"></span>&nbsp&nbspPROCEDIMIENTOS SERVICIO VEHICULO
				</h3>
			</div>
			<!-- Contenido -->
			<div class="modal-body">
		    	<div class="container-fluid">
					<object width="100%" height="50px" data="{{ asset('document/servicios.pdf') }}"></object>
		    	</div>
		    </div>
	    	<!-- Pie -->
			<div class="modal-footer">
	...
	<div id="modalfiltros" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
		    ...
		    <div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-filter"></span>&nbsp&nbspFILTROS SERVICIO VEHICULO
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					{!!Form::open(['route'=>'servicios.index','method'=>'get'])!!}
			    	<div class="container-fluid">
			    		<!-- CAJA DE TEXTO BUSQUEDA: NOMBRE -->	
			    		<div class="row form-group">
			    			<label class="col-sm-3 text-right control-label">Servicio:</label>
			    			<div class="col-md-9">
				...
				<div class="row"> 
					...
					<div class="col-md-2 text-right" style="padding-left: 9px">
						...
						@if(Auth::user()->m_capreep=="SI")
							<form action="{{ route('servicios-pdf') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
					...
					<div class="col-md-3 text-left" style="padding-left: 25px">
						...
						@if(Auth::user()->m_capreex=="SI")
							<form action="{{ route('servicios-excel') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
	...
	<script>
		...
		var anc_bar_pro = 0;
		var tot_reg_hal = {{$servicios->total()}};
		var tot_reg_pen = {{$servicios->total()}};

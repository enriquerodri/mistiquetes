...
//Descripción:      Página principal: Tipos de carroceria para clasificación vehículos
//Actualizaciones
...
@extends('template')
@section('title','Tipos de carroceria vehículos')
@include('sweet::alert')
...
@include('partials.menu-vehiculos')
	<div class="container text-center">
		<!-- ADMINISTRATIVOS - TIPOS DE CARROCERIA DE VEHICULOS -->
		<hr class="separador">
    		<h3>TIPOS DE CARROCERIA VEHICULOS</h3>
    	<hr class="separador">
		...
		<div class="row text-center">
			...
			<div class="col-md-3">
				...
				<form action="{{ route('tipos-carrocerias.index') }}" method="get">
					<!-- ORDENAMIENTO -->
					...
					<div class="col-md-5 text-left" style="padding-left: 0px">
						<button 
							...
							</i>&nbsp&nbsp&nbsp(&nbsp&nbspTotal Registros {{count($tiposcarrocerias)}} de {{$tiposcarrocerias->total()}}&nbsp&nbsp)
						</button>
					</div>
				</form>
			</div>
			<!-- PAGINADO SUPERIOR -->
			@if($tiposcarrocerias->total()>160)
				<div class="col-md-12 text-right">
			@else
				<div class="col-md-4 text-right">
			@endif
				{{$tiposcarrocerias->appends([
					...
					<tr>
						<td>
							...
							@if(Auth::user()->m_capreed=="SI")
								<a onclick="consultar({{$tipocarroceria->m_nucodig}})" id="btneditar" class="btn btn-primary" data-toggle="modal" data-target="#modaleditar" title="Editar registro" data-placement="right"><i class="fa fa-edit" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>
								</a>
							@else
						...
						<td class="text-left">{{$tipocarroceria->m_canombr}}</td>
						<td>{{$tipocarroceria->m_cacodna}}</td>
						<td>{{$tipocarroceria->m_caestad}}</td>
						<td>{{$tipocarroceria->created_at}}</td>
						<td>{{$tipocarroceria->usuario_registra->name}}</td>
						<td>{{$tipocarroceria->updated_at}}</td>
						<td>{{$tipocarroceria->usuario_actualiza->name}}</td>
					</tr>
				@endforeach
			</tbody>
			<!-- FIN CONTENIDO -->
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-check"></span>&nbsp&nbspCREAR TIPO DE CARROCERIA VEHICULO
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					...
					{!!Form::open(['route'=>'tipos-carrocerias.store','class'=>'form-horizontal']) !!}
		...
		<div id="modaleditar" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-header" style="background-color: #3498db">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-edit"></span>&nbsp&nbspEDITANDO TIPO DE CARROCERIA DE VEHICULO
							...
							<div class="modal-footer">
								<div class="text-center">
									...
									@if(Auth::user()->m_capreel=="SI")
										<a onclick="eliminar({{$tipocarroceria->m_nucodig}})" id="btn btn-danger" class="btn btn-danger" data-toggle="modal" data-target="#modaleditar" title="" data-placement="right"><i class="fa fa-trash" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>&nbsp&nbspEliminar
										</a>
									@else
	    ...
	    function consultar(id){
	    	...
	    	$.ajax({
                ...
                success:function(data){
                    ...
                    $("#formeliminar").attr("action","tipos-carrocerias/"+id+"");
                }               
            })     
	    }
	    ...
	    <div class="modal-content">
			...
			<div class="modal-body">
		    	<div class="container-fluid">
					<object width="100%" height="50px" data="{{ asset('document/tipos-carrocerias.pdf') }}"></object>
		    	</div>
		    </div>
	    	<!-- Pie -->
			<div class="modal-footer">
	...
	<div id="modalfiltros" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
		    ...
		    <div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-filter"></span>&nbsp&nbspFILTROS TIPO DE CARROCERIA
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					{!!Form::open(['route'=>'tipos-carrocerias.index','method'=>'get'])!!}
			    	<div class="container-fluid">
			    		<!-- CAJA DE TEXTO BUSQUEDA: NOMBRE -->	
			    		<div class="row form-group">
			    			<label class="col-sm-3 text-right control-label">Carrocería:</label>
			    			<div class="col-md-9">
			    				<div class="icon-addon addon-lg">
				    				{!! Form::text('m_canombr',null, [
										...
										'title'=>"Tipo de carrocería (5 a 50 caracteres - A-Z Ñ 0-9 #-_.&/)",
										'maxlength'=>'50',
				...
				<div class="row"> 
					...
					<div class="col-md-2 text-right" style="padding-left: 9px">
						...
						@if(Auth::user()->m_capreep=="SI")
							<form action="{{ route('tipos-carrocerias-pdf') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
					...
					<div class="col-md-3 text-left" style="padding-left: 25px">
						...
						@if(Auth::user()->m_capreex=="SI")
							<form action="{{ route('tipos-carrocerias-excel') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
	...
	<script>
		...
		var anc_bar_pro = 0;
		var tot_reg_hal = {{$tiposcarrocerias->total()}};
		var tot_reg_pen = {{$tiposcarrocerias->total()}};

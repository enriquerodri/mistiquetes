...
<!-- TITULO DOCUMENTOS VEHICULO -->
<h4 class="modal-title"
	id="myModalLabel"
	style="text-align: center; font-size:16px; color: #0E2FF5; margin-bottom: 10px;">
	<span class="fa fa-file-text-o"></span>&nbsp&nbspDOCUMENTOS VEHICULO
</h4>
<!-- BOTON AGREGAR -->
<div class="form-group">
	<div class="col-sm-2">
		<a id="btnagregardocumento"
			...
			onclick="agregarDocumentoVehiculo()"
...
<div id="table-responsive">
	<table class="table table-striped table-hover table-bordered" id="tablaDocumentosVehiculos">
		<!-- INICIO ENCABEZADO -->
		<thead>
...
<div class="editartablaDocumentosVehiculos" hidden="hidden">
	<!-- DOCUMENTO NOMBRE -->
	<h4 class="modal-title"
		id="myModalLabel"
	...
	$('#accordion12b').on('show.bs.collapse hide.bs.collapse', function (e) {
	  if (e.type == 'show') {
	   //alert("abierto")
		...
		$(".editartablaDocumentosVehiculos").hide()
	  }
	})
...
<script>
	...
	$( "#modaleditar #guardarDocumentoVehiculo" ).click(function(e) {
	   //alert(this.id);
        ...
        //NOMBRE VEHICULO
        var nombre_conductor_d = $('#modaleditar #m_caplaca').val()
        //
		...
		var agregarDocumentoVehiculo = false;
		...
		var data={
			...
			m_cacateg: m_cacateg,
			m_cadetal: m_cadetal,
			agregarDocumentoVehiculo: agregarDocumentoVehiculo,
			m_nucocie: m_nucocie,_token:token};

			var host = "/ticket/public"
  	
		$.ajax({
            url: host + '/guardarDocumentoVehiculo',
            type: 'POST',
            data: data,
            dataType: 'JSON',
            success: function (data) {
	...
	function agregarDocumentoVehiculo(){
		...
		var codigoVehiculo   = $("#modaleditar #m_id").val();
		var nombreDocumento = $("#modaleditar #m_nucodigo_d option:selected").text();
		var agregarDocumentoVehiculo = true;
        //NOMBRE CONDUCTOR
        var nombre_conductor_d = $('#modaleditar #m_caplaca').val()
        //
		var token = '{{csrf_token()}}';// ó $("#token").val() si lo tienes en una etiqueta html.
								...
								$('#modaleditar #tablaDocumentosVehiculos tbody')
								    .append($('<tr>')
										.append($('<td>').append($('<button>')
											             .attr('id',data[0].m_nucodig+'editartablaDocumentosVehiculos')
											             .attr('type','button')
											             .attr('class','btn btn-primary')
											             .addClass('fa fa-edit')
											             .attr('style','font-size:18px;text-shadow:2px 2px 4px #000000;')
											             .attr('title','Editar registros')
	...
	$( "#modaleditar #eliminarDocumentoVehiculo" ).click(function(e) {
	   //alert(this.id);
		...
		swal({
			...
			if (result) {
				$.ajax({
					url: host + '/eliminarDocumentoVehiculo',
					type: 'POST',
					data: data,
					dataType: 'JSON',
					success: function (data) {

						$("#" + id + "editartablaDocumentosVehiculos").parents("tr").remove();

...
<!-- TITULO SEGURO VEHICULO --> 
<h4 class="modal-title"
	id="myModalLabel"
	style="text-align: center; font-size:16px; color: #0E2FF5; margin-bottom: 10px;">
	<span class="fa fa-thumbs-o-up"></span>&nbsp&nbspSEGUROS VEHICULO
</h4>
<!-- BOTON AGREGAR -->
...
<div id="table-responsive">
	<table class="table table-striped table-hover table-bordered" id="tablaSegurosVehiculos">
...
<div class="editartablaSegurosVehiculos" hidden="hidden">
	<!-- SEGURO NOMBRE -->
	<h4 class="modal-title"
		id="myModalLabel"
	...
	$('#accordion13b').on('show.bs.collapse hide.bs.collapse', function (e) {
	  if (e.type == 'show') {
	   //alert("abierto")
		...
		$(".editartablaSegurosVehiculos").hide()
	  }
	})
...
<script>
	...
	$( "#modaleditar #guardarSeguroVehiculo" ).click(function(e) {
	   //alert(this.id); 
        ...
        //NOMBRE VHICULO
        var nombre_conductor_s = $('#modaleditar #m_caplaca').val()
        //
        ...
        var agregarSeguroVehiculo = false;
		...
		var data={
			...
			m_cacateg: m_cacateg,
			m_cadetal: m_cadetal,
			agregarSeguroVehiculo: agregarSeguroVehiculo,
			m_nucocie: m_nucocie,_token:token};

			var host = "/ticket/public"

		$.ajax({
            url: host + '/guardarSeguroVehiculo',
            type: 'POST',
            data: data,
            dataType: 'JSON',
            success: function (data) {               		
	...
	function agregarSeguroVehiculo(){
		...
		var agregarSeguroVehiculo = true;
        //NOMBRE CONDUCTOR
        var nombre_conductor_s = $('#modaleditar #m_caplaca').val()
        //
		var token = '{{csrf_token()}}';// ó $("#token").val() si lo tienes en una etiqueta html.
								...
								$('#modaleditar #tablaSegurosVehiculos tbody')
								    .append($('<tr>')
										.append($('<td>').append($('<button>')
											             .attr('id',data[0].m_nucodig+'editarTablaSegurosVehiculos')
											             .attr('type','button')
	...
	$( "#modaleditar #eliminarSeguroVehiculo" ).click(function(e) {
	   //alert(this.id);
		...
		swal({
			...
			if (result) {
				$.ajax({
		           	url: host + '/eliminarSeguroVehiculo',
		            type: 'POST',
		            data: data,
		            dataType: 'JSON',
		            success: function (data) {

						$("#" + id + "editarTablaSegurosVehiculos").parents("tr").remove();

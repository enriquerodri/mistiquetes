...
//Descripción:      Página principal: Rutas registradas organización empresarial
//Actualizaciones
...
@include('partials.menu-vehiculos')
	<div class="container text-center">
		...
		<div class="row text-center">
			...
			<div class="col-md-3">
				...
				<form action="{{ route('rutas-registro.index') }}" method="get">
					<!-- ORDENAMIENTO -->
					...
					<div class="col-md-5 text-left" style="padding-left: 0px">
						<button 
							...
							</i>&nbsp&nbsp&nbsp(&nbsp&nbspTotal Registros {{count($rutas)}} de {{$rutas->total()}}&nbsp&nbsp)
						</button>
					</div>
				</form>
			</div>
					...
					<tr>
						<td>
							...
							@if(Auth::user()->m_capreed=="SI")
								<a onclick="consultar({{$ruta->m_nucodig}});datosCiudadOrigen({{$ruta->m_nucoori}});datosCiudadDestino({{$ruta->m_nucodes}});" id="btneditar" class="btn btn-primary" data-toggle="modal" data-target="#modaleditar" title="Editar registro" data-placement="right"><i class="fa fa-edit" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>
								</a>
							@else
						...
						<td class="text-left">{{$ruta->m_canombr}}</td>
						<td>{{$ruta->m_cacodna}}</td>
						<td>{{$ruta->m_caestad}}</td>
						<td>{{$ruta->created_at}}</td>
						<td>{{$ruta->usuario_registra->name}}</td>
						<td>{{$ruta->updated_at}}</td>
						<td>{{$ruta->usuario_actualiza->name}}</td>
					</tr>
				@endforeach
			</tbody>
			<!-- FIN CONTENIDO -->
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-check"></span>&nbsp&nbspCREAR RUTA
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					...
					{!!Form::open(['route'=>'rutas.store','class'=>'form-horizontal']) !!}
		...
		<div id="modaleditar" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-header" style="background-color: #3498db">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-edit"></span>&nbsp&nbspEDITANDO RUTA
							...
							<div class="modal-footer">
								<div class="text-center">
									...
									@if(Auth::user()->m_capreel=="SI")
										<a onclick="eliminar({{$ruta->m_nucodig}})" id="btn btn-danger" class="btn btn-danger" data-toggle="modal" data-target="#modaleditar" title="" data-placement="right"><i class="fa fa-trash" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>&nbsp&nbspEliminar
										</a>
									@else
	...
	<script>
	    function consultar(id){
	    	...
	    	$.ajax({
                ...
                success:function(data){
                    ...
                    console.log(data);
                    $("#formeditar").attr("action","rutas/"+id+"");
                    $('#modaleditar #m_id').val(data.m_nucodig);
                    $('#modaleditar #m_canombr').val(data.m_canombr);
                    $('#modaleditar #m_camapas').val(data.m_camapas);
                    $('#modaleditar #m_caestad').val(data.m_caestad);
                    $('#modaleditar #m_cacodna').val(data.m_cacodna);
                    $('#modaleditar #m_nucoori').val(data.m_nucoori);
                    $('#modaleditar #m_nucodes').val(data.m_nucodes);
                    $('#modaleditar #m_cacondu').val(data.m_cacondu);
                    $('#modaleditar #m_nukilom').val(data.m_nukilom);
                    $('#modaleditar #m_catiemp').val(data.m_catiemp);
                    $('#modaleditar #m_catipos').val(data.m_catipos);
                    $('#modaleditar #m_caclase').val(data.m_caclase);
                    ...
                    $("#formeliminar").attr("action","rutas/"+id+"");

					//0- DATOS MAPA
		        	var dato_00 = ""
		        	var dato_01 = ""
		        	//
		        	var dato_00 = $('#modaleditar #m_canombr').val();
		        	var dato_01 = $('#modaleditar #m_camapas').val();
					//1- ETIQUETA RUTA
					$('#modaleditar #mapa_ruta').attr({
					   'title':'RUTA: ' + dato_00,
					   'href': "https://www.google.com/maps/dir/" + dato_01,
		...
		function datosCiudadOrigen(id){
			...
			$.ajax({
		        ...
		        success:function(datados){
		            console.log(datados);
		            $('#modaleditar #datociud_origen').val(datados[0]);
		            $('#modaleditar #datoDepa_origen').val(datados[1]);
		            $('#modaleditar #datoPais_origen').val(datados[2]);
		...
		function datosCiudadDestino(id){
			...
			$.ajax({
		        ...
		        success:function(datados){
		            console.log(datados);
		            $('#modaleditar #datociud_destino').val(datados[0]);
		            $('#modaleditar #datoDepa_destino').val(datados[1]);
		            $('#modaleditar #datoPais_destino').val(datados[2]);
		            $('#modaleditar #datoInd1').val(datados[5]);
		            $('#modaleditar #datoInd2').val(datados[6]);
		        }               
		    })
	...
	<div id="modalprocedimientos" class="modal fade">
	    ...
	    <div class="modal-content">
			...
			<div class="modal-header" style="background-color: #3498db">
				...
				<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-list-alt"></span>&nbsp&nbspPROCEDIMIENTOS RUTA
				</h3>
			</div>
			<!-- Contenido -->
			<div class="modal-body">
		    	<div class="container-fluid">
					<object width="100%" height="50px" data="{{ asset('document/rutas.pdf') }}"></object>
		    	</div>
		    </div>
	    	<!-- Pie -->
			<div class="modal-footer">
	...
	<div id="modalfiltros" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
		    ...
		    <div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-filter"></span>&nbsp&nbspFILTROS RUTA
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
			    			...
			    			<div class="col-md-9">
			    				<div class="icon-addon addon-lg">
				    				{!! Form::text('m_canombr',null, [
										...
										'title'=>"Ruta (5 a 50 caracteres - A-Z Ñ 0-9 #-_.&/)",
										'maxlength'=>'50',
				...
				<div class="row"> 
					...
					<div class="col-md-2 text-right" style="padding-left: 9px">
						...
						@if(Auth::user()->m_capreep=="SI")
							<form action="{{ route('rutas-pdf') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
					...
					<div class="col-md-3 text-left" style="padding-left: 25px">
						...
						@if(Auth::user()->m_capreex=="SI")
							<form action="{{ route('rutas-excel') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
	...
	<script>
		...
		var anc_bar_pro = 0;
		var tot_reg_hal = {{$rutas->total()}};
		var tot_reg_pen = {{$rutas->total()}};

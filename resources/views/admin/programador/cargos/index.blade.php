...
@include('partials.menu-programador')
		...
		<div class="row text-center">
			...
			<div class="col-md-3">
				...
				<form action="{{ route('cargos.index') }}" method="get">
					...
					<input type="hidden" name="m_caticar" value="<?php if(isset($_GET['m_caticar'])) echo $_GET['m_caticar']; ?>">
					<input type="hidden" name="m_nucodiv" value="<?php if(isset($_GET['m_nucodiv'])) echo $_GET['m_nucodiv']; ?>">
					...
					<div class="col-md-5 text-left" style="padding-left: 0px">
						<button 
							...
							</i>&nbsp&nbsp&nbsp(&nbsp&nbspTotal Registros {{count($cargos)}} de {{$cargos->total()}}&nbsp&nbsp)
						</button>
					</div>
				</form>
			</div>
			...
			@endif
				...
				'm_caticar'=>$m_caticar_filtro,
				'm_nucodiv'=>$m_nucodiv_filtro,
					...
					<tr>
						<td>
							...
							@if(Auth::user()->m_capreed=="SI")
								<a onclick="consultar({{$cargo->m_nucodig}})"
									id="btneditar"
									...
									<i class="fa fa-edit" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>
								</a>
							@else
						...
						<td class="text-left">{{$cargo->m_canombr}}</td>
						<td>{{$cargo->m_caticar}}</td>
						<td>{{$cargo->division->m_canombr}}</td>
						<td>{{$cargo->m_cacodna}} </td>
						<td>{{$cargo->m_caestad}} </td>
						<td>{{$cargo->created_at}} </td>
						<td>{{$cargo->usuario_registra->name}} </td>
						<td>{{$cargo->updated_at}} </td>
						<td>{{$cargo->usuario_actualiza->name}} </td>
					</tr>
				@endforeach
			</tbody>
			<!-- FIN CONTENIDO -->
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-check"></span>&nbsp&nbspCREAR CARGO
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					...
					@include('admin.programador.cargos.form')
				</div>
				<!-- Pie -->
		...
		<div id="modaleditar" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-header" style="background-color: #3498db">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-edit"></span>&nbsp&nbspEDITANDO CARGO
							...
							<div class="modal-footer">
								<div class="text-center">
									...
									@if(Auth::user()->m_capreel=="SI")
										<a onclick="eliminar({{$cargo->m_nucodig}})" id="btn btn-danger" class="btn btn-danger" data-toggle="modal" data-target="#modaleditar" title="" data-placement="right"><i class="fa fa-trash" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>&nbsp&nbspEliminar
										</a>
									@else
	    ...
	    function consultar(id){
	    	...
	    	$.ajax({
                ...
                success:function(data){
                    ...
                    console.log(data);
                    $("#formeditar").attr("action","cargos/"+id+"");
                    //AGREGAR EN TODOS LOS INDEX - SOLUCION AL ELIMINAR REGISTRO
                    $('#modaleditar #m_id').val(data.m_nucodig);
                    $('#modaleditar #m_canombr').val(data.m_canombr);
                    $('#modaleditar #m_caticar').val(data.m_caticar);
                    $('#modaleditar #m_nucodiv').val(data.m_nucodiv);
                    ...
                    $("#formeliminar").attr("action","cargos/"+id+"");
                }               
            })     
	    }
	...
	<div id="modalprocedimientos" class="modal fade">
	    ...
	    <div class="modal-content">
			...
			<div class="modal-header" style="background-color: #3498db">
				...
				<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-list-alt"></span>&nbsp&nbspPROCEDIMIENTOS CARGO
				</h3>
			</div>
			<!-- Contenido -->
			<div class="modal-body">
		    	<div class="container-fluid">
					<object width="100%" height="50px" data="{{ asset('document/cargos.pdf') }}"></object>
		    	</div>
		    </div>
	    	<!-- Pie -->
			<div class="modal-footer">
				<div class="text-center">
					<button type="button" 
						...
						data-dismiss="modal"
						style="text-shadow:2px 2px 4px #000000;">
						<i class="fa fa-close" style="font-size:18px;text-shadow:2px 2px 4px #000000;"></i>&nbsp&nbspCerrar
					</button>
				</div>
			</div>
	    </div>
	...
	<div id="modalfiltros" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
		    ...
		    <div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-filter"></span>&nbsp&nbspFILTROS CARGO
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					{!!Form::open(['route'=>'cargos.index','method'=>'get'])!!}
			    	<div class="container-fluid">
			    		<!-- CAJA DE TEXTO BUSQUEDA: NOMBRE -->
			    		<div class="row form-group">
			    			<label class="col-sm-3 text-right control-label">Cargo:</label>
			    			<div class="col-md-9">
			    				<div class="icon-addon addon-lg">
				    				{!! Form::text('m_canombr',null, [
										...
										'title'=>"Nombre Cargo (5 a 50 caracteres - A-Z Ñ 0-9 #-_.&/)",
										'maxlength'=>'50',
						...
						<!-- LISTA: TIPO -->
						<div class="row form-group">
							<label class="col-sm-3 text-right control-label">Tipo:</label>
						    <div class="col-md-9">
								<div class="icon-addon addon-md">
									{!! Form::select('m_caticar', ['EMPLEADO'=>'EMPLEADO','CONTRATISTA'=>'CONTRATISTA'], null, [
											'class'=>'form-control',
											'id'=>'m_caticar',
											'placeholder'=>'TIPO',
									...
									!!}
									<label for="m_caticar" class="glyphicon glyphicon-search" rel="tooltip" title="Seleccionar Tipo">
									</label>
								</div>
							</div>
						</div>
						...
						<!-- LISTA: DIVISION -->
						<div class="row form-group">
							<label class="col-sm-3 text-right control-label">División:</label>
						    <div class="col-md-9">
								<div class="icon-addon addon-md">
									{!! Form::select('m_nucodiv',$divisiones,null, [
											'class'=>'form-control',
											'id'=>'m_nucodiv',
											'placeholder'=>'DIVISION',
											'title'=>"Lista de selección División"
										])
									!!}
									<label for="m_nucodiv" class="glyphicon glyphicon-search" rel="tooltip" title="Seleccionar Tipo">
									</label>
								</div>
							</div>
						</div>
				...
				<div class="row">
					<div class="col-md-6 text-right" style="padding-right: 0px">
						...
						@if(Auth::user()->m_capreep=="SI")
							<form action="{{ route('cargos-pdf') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
					...
					<div class="col-md-6 text-left" style="padding-left: 8px">
						...
						@if(Auth::user()->m_capreex=="SI")
							<form action="{{ route('cargos-excel') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
								...
								<button 
									...
									data-toggle="collapse" data-target="#mibarra"
									onclick="avance_tarea()">
									<i class="fa fa-file-excel-o fa-1x"style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>&nbsp&nbsp Exportar a hoja de calculo
								</button>
							</form>
						@else
	...
	<script>
		...
		var anc_bar_pro = 0;
		var tot_reg_hal = {{$cargos->total()}};
		var tot_reg_pen = {{$cargos->total()}};

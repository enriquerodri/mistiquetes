...
//Descripción:      Página principal: Clientes convenios empresariales
//Actualizaciones
...
@extends('template')
@section('title','Clientes convenios')
@include('sweet::alert')
...
@include('partials.menu-programador')
	<div class="container text-center">
		<!-- ADMINISTRATIVOS - CLIENTES CONVENIOS -->
		<hr class="separador">
    		<h3>CLIENTES CONVENIOS</h3>
    	<hr class="separador">
		...
		<div class="row text-center">
			...
			<div class="col-md-3">
				...
				<form action="{{ route('clientesconvenios.index') }}" method="get">
					<!-- ORDENAMIENTO -->
					<!-- ASCENDENTE - DESCENTE -->
					...
					<div class="col-md-5 text-left" style="padding-left: 0px">
						<button 
							...
							</i>&nbsp&nbsp&nbsp(&nbsp&nbspTotal Registros {{count($convenios)}} de {{$convenios->total()}}&nbsp&nbsp)
						</button>
					</div>
				</form>
			</div>
			<!-- PAGINADO SUPERIOR -->
			@if($convenios->total()>160)
					...
					<tr>
						<td>
							...
							@if(Auth::user()->m_capreed=="SI")
								<a onclick="consultar({{$convenio->m_nucodig}});datosC({{$convenio->m_nucociu}});" id="btneditar" class="btn btn-primary" data-toggle="modal" data-target="#modaleditar" title="Editar registro" data-placement="right"><i class="fa fa-edit" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>
								</a>
							@else
						...
						<td class="text-left">{{$convenio->m_canuipe}}</td>
						<td>{{$convenio->m_canombr}}</td>
						<td>{{$convenio->m_cadirec}}</td>
						<td>{{$convenio->ciudades->m_canombr}} - {{$departamento_x[$j]}} - {{$pais_x[$j]}}</td>
						<td>{{$convenio->m_catelfi}}</td>
						<td>{{$convenio->m_camovil}}</td>
						<td>{{$convenio->m_cadacon}}</td>
						<td>{{$convenio->m_cacodna}}</td>
						<td>{{$convenio->m_caestad}}</td>
						<td>{{$convenio->created_at}} </td>
						<td>{{$convenio->usuario_registra->name}} </td>
						<td>{{$convenio->updated_at}} </td>
						<td>{{$convenio->usuario_actualiza->name}} </td>
					</tr>
					<?php $j++; ?>
				@endforeach
			</tbody>
			<!-- FIN CONTENIDO -->
			{{-- <td>{{$convenio->departamentos->m_canombr}}</td>
			<td>{{$convenio->paises->m_canombr}}</td> --}}
		</table>
		</div>
		<!-- FIN TABLA -->
		<!-- PAGINADO INFERIOR -->
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-check"></span>&nbsp&nbspCREAR CLIENTE CONVENIO
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					...
					{!!Form::open(['route'=>'clientesconvenios.store','class'=>'form-horizontal']) !!}
					@include('admin.programador.clientesconvenios.form')
				</div>
				<!-- Pie -->
		...
		<div id="modaleditar" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-header" style="background-color: #3498db">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-edit"></span>&nbsp&nbspEDITANDO CLIENTE CONVENIO
	    ...
	    function consultar(id){
	    	...
	    	$.ajax({
                ...
                success:function(data){
                    ...
                    $("#formeliminar").attr("action","clientesconvenios/"+id+"");
	...
	<div id="modalprocedimientos" class="modal fade">
	    ...
	    <div class="modal-content">
			...
			<div class="modal-header" style="background-color: #3498db">
				...
				<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-list-alt"></span>&nbsp&nbspPROCEDIMIENTOS CLIENTE CONVENIO
				</h3>
			</div>
			<!-- Contenido -->
			<div class="modal-body">
		    	<div class="container-fluid">
					<object width="100%" height="50px" data="{{ asset('document/convenios.pdf') }}"></object>
		    	</div>
		    </div>
	    	<!-- Pie -->
			<div class="modal-footer">
		...
		<div class="modal-dialog modal-lg">
		    <div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-filter"></span>&nbsp&nbspFILTROS CLIENTE CONVENIO
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					{!!Form::open(['route'=>'clientesconvenios.index','method'=>'get'])!!}
			    	<div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-6 text-right" style="padding-right: 0px">
						...
						@if(Auth::user()->m_capreep=="SI")
							<form action="{{ route('clientesconvenios-pdf') }}" method="get">
								<!-- ORDENAMIENTO -->
					...
					<div class="col-md-6 text-left" style="padding-left: 8px">
						...
						@if(Auth::user()->m_capreex=="SI")
							<form action="{{ route('clientesconvenios-excel') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
	...
	<script>
		...
		var anc_bar_pro = 0;
		var tot_reg_hal = {{$convenios->total()}};
		var tot_reg_pen = {{$convenios->total()}};

...
//Descripción:      Página principal: Terminales de transporte
//Actualizaciones
...
@include('partials.menu-programador')
	<div class="container text-center">
		...
		<hr class="separador">
    		<h3>TERMINALES DE TRANSPORTE</h3>
    	<hr class="separador">
		...
		<div class="row text-center">
			...
			<div class="col-md-3">
				...
				<form action="{{ route('terminales.index') }}" method="get">
					<!-- ORDENAMIENTO -->
					<!-- ASCENDENTE - DESCENTE -->
					...
					<div class="col-md-5 text-left" style="padding-left: 0px">
						<button 
							...
							</i>&nbsp&nbsp&nbsp(&nbsp&nbspTotal Registros {{count($terminales)}} de {{$terminales->total()}}&nbsp&nbsp)
						</button>
					</div>
				</form>
			</div>
			<!-- PAGINADO SUPERIOR -->
			@if($terminales->total()>160)
					...
					<tr>
						<td>
							...
							@if(Auth::user()->m_capreed=="SI")
								<a onclick="consultar({{$terminal->m_nucodig}});datosC({{$terminal->m_nucociu}});" id="btneditar" class="btn btn-primary" data-toggle="modal" data-target="#modaleditar" title="Editar registro" data-placement="right"><i class="fa fa-edit" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>
								</a>
							@else
						...
						<td class="text-left">{{$terminal->m_canuipe}}</td>
						<td>{{$terminal->m_canombr}}</td>
						<td>{{$terminal->m_cadirec}}</td>
						<td>{{$terminal->ciudades->m_canombr}} - {{$departamento_x[$j]}} - {{$pais_x[$j]}}</td>
						<td>{{$terminal->m_catelfi}}</td>
						<td>{{$terminal->m_camovil}}</td>
						<td>{{$terminal->m_cadacon}}</td>
						<td>{{$terminal->m_cacodna}}</td>
						<td>{{$terminal->m_caestad}}</td>
						<td>{{$terminal->created_at}} </td>
						<td>{{$terminal->usuario_registra->name}} </td>
						<td>{{$terminal->updated_at}} </td>
						<td>{{$terminal->usuario_actualiza->name}} </td>
					</tr>
					<?php $j++; ?>
				@endforeach
			</tbody>
			<!-- FIN CONTENIDO -->
			{{-- <td>{{$terminal->departamentos->m_canombr}}</td>
			<td>{{$terminal->paises->m_canombr}}</td> --}}
		</table>
		</div>
		<!-- FIN TABLA -->
		<!-- PAGINADO INFERIOR -->
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-check"></span>&nbsp&nbspCREAR TERMINAL DE TRANSPORTE
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					...
					{!!Form::open(['route'=>'terminales.store','class'=>'form-horizontal']) !!}
					@include('admin.programador.terminales.form')
				</div>
				<!-- Pie -->
		...
		<div id="modaleditar" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-header" style="background-color: #3498db">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-edit"></span>&nbsp&nbspEDITANDO TERMINAL DE TRANSPORTE
	    ...
	    function consultar(id){
	    	...
	    	$.ajax({
                ...
                success:function(data){
                    ...
                    $("#formeliminar").attr("action","terminales/"+id+"");

					//	MODAL EDITAR
					//0- DIRECCION
					//1- NOMBRE
					var dato_01 = $("#modaleditar #m_canombr").val()
					////6- MAPA DIRECCION
					$("#modaleditar #m_camapas").val(dato_01);
					//7- ETIQUETA DIRECCION
					$('#modaleditar #mapa_ruta').attr({
					   'title':'UBICACION: ' + dato_01,
					   'href': "https://www.google.com/maps/search/" + dato_01,
	...
	<div id="modalprocedimientos" class="modal fade">
	    ...
	    <div class="modal-content">
			...
			<div class="modal-header" style="background-color: #3498db">
				...
				<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-list-alt"></span>&nbsp&nbspPROCEDIMIENTOS TERMINAL DE TRANSPORTE
				</h3>
			</div>
			<!-- Contenido -->
			<div class="modal-body">
		    	<div class="container-fluid">
					<object width="100%" height="50px" data="{{ asset('document/terminales.pdf') }}"></object>
		    	</div>
		    </div>
	    	<!-- Pie -->
			<div class="modal-footer">
		...
		<div class="modal-dialog modal-lg">
		    <div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-filter"></span>&nbsp&nbspFILTROS TERMINAL DE TRANSPORTE
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					{!!Form::open(['route'=>'terminales.index','method'=>'get'])!!}
			    	<div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-6 text-right" style="padding-right: 0px">
						...
						@if(Auth::user()->m_capreep=="SI")
							<form action="{{ route('terminalesdatos-pdf') }}" method="get">
								<!-- ORDENAMIENTO -->
					...
					<div class="col-md-6 text-left" style="padding-left: 8px">
						...
						@if(Auth::user()->m_capreex=="SI")
							<form action="{{ route('terminalesdatos-excel') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
	...
	<script>
		...
		var anc_bar_pro = 0;
		var tot_reg_hal = {{$terminales->total()}};
		var tot_reg_pen = {{$terminales->total()}};

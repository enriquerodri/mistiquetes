...
//Descripción:      Página principal: Conceptos activaciones o inactivaciones (humanos - robots - vehículos)
//Actualizaciones
...
@include('partials.menu-programador')
	<div class="container text-center">
		...
		<hr class="separador">
    		<h3>CONCEPTOS ACTIVACIONES - INACTIVACIONES</h3>
    	<hr class="separador">
		...
		<div class="row text-center">
			...
			<div class="col-md-3">
				...
				<form action="{{ route('conceptos.index') }}" method="get">
					...
					<input type="hidden" name="m_caapesv" value="<?php if(isset($_GET['m_caapesv'])) echo $_GET['m_caapesv']; ?>">
					...
					<div class="col-md-5 text-left" style="padding-left: 0px">
						<button 
							...
							</i>&nbsp&nbsp&nbsp(&nbsp&nbspTotal Registros {{count($conceptos)}} de {{$conceptos->total()}}&nbsp&nbsp)
						</button>
					</div>
				</form>
			</div>
			<!-- PAGINADO SUPERIOR -->
			@if($conceptos->total()>160)
			...
			@endif
	            ...
	            'm_caaplic'=>$m_caaplic_filtro,
	            'm_cacondi'=>$m_cacondi_filtro,
					...
					<tr>
						<td>
							...
							@if(Auth::user()->m_capreed=="SI")
								<a onclick="consultar({{$concepto->m_nucodig}})" id="btneditar" class="btn btn-primary" data-toggle="modal" data-target="#modaleditar" title="Editar registro" data-placement="right"><i class="fa fa-edit" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>
								</a>
							@else
						...
						<td class="text-left">{{$concepto->m_canombr}}</td>
						<td>{{$concepto->m_caaplic}}</td>
						<td>{{$concepto->m_cacondi}}</td>
						<td>{{$concepto->m_caapesv}}</td>
						<td>{{$concepto->m_cacodna}}</td>
						<td>{{$concepto->m_caestad}}</td>
						<td>{{$concepto->created_at}}</td>
						<td>{{$concepto->usuario_registra->name}}</td>
						<td>{{$concepto->updated_at}}</td>
						<td>{{$concepto->usuario_actualiza->name}}</td>
					</tr>
				@endforeach
			</tbody>
			<!-- FIN CONTENIDO -->
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-check"></span>&nbsp&nbspCREAR CONCEPTO
					</h3>
				</div>
				<!-- Contenido -->
		...
		<div id="modaleditar" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-header" style="background-color: #3498db">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-edit"></span>&nbsp&nbspEDITANDO CONCEPTO
							...
							<div class="modal-footer">
								<div class="text-center">
									...
									@if(Auth::user()->m_capreel=="SI")
										<a onclick="eliminar({{$concepto->m_nucodig}})" id="btn btn-danger" class="btn btn-danger" data-toggle="modal" data-target="#modaleditar" title="" data-placement="right"><i class="fa fa-trash" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>&nbsp&nbspEliminar
										</a>
									@else
	    ...
	    function consultar(id){
	    	...
	    	$.ajax({
                ...
                success:function(data){
                    ...
                    $("#formeliminar").attr("action","conceptos/"+id+"");
                }               
            })     
	    }
	...
	<div id="modalprocedimientos" class="modal fade">
	    ...
	    <div class="modal-content">
			...
			<div class="modal-header" style="background-color: #3498db">
				...
				<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-list-alt"></span>&nbsp&nbspPROCEDIMIENTOS CONCEPTO
				</h3>
			</div>
			<!-- Contenido -->
			<div class="modal-body">
		    	<div class="container-fluid">
					<object width="100%" height="50px" data="{{ asset('document/conceptos.pdf') }}"></object>
		    	</div>
		    </div>
	    	<!-- Pie -->
			<div class="modal-footer">
	...
	<div id="modalfiltros" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
		    ...
		    <div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-filter"></span>&nbsp&nbspFILTROS CONCEPTO
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					{!!Form::open(['route'=>'conceptos.index','method'=>'get'])!!}
			    	<div class="container-fluid">
			    		<!-- CAJA DE TEXTO BUSQUEDA: NOMBRE -->	
			    		<div class="row form-group">
			    			<label class="col-sm-3 text-right control-label">Concepto:</label>
			    			<div class="col-md-9">
			    				<div class="icon-addon addon-lg">
				    				{!! Form::text('m_canombr',null, [
										...
										'title'=>"Concepto (5 a 50 caracteres - A-Z Ñ 0-9 #-_.&/)",
										'maxlength'=>'50',
						...
						<!-- LISTA: APLICA A -->
						<div class="row form-group">
							<label class="col-sm-3 text-right control-label">Aplica a:</label>
						    <div class="col-md-9">
								<div class="icon-addon addon-md">
									{!! Form::select('m_caaplic', ['PERSONA'=>'PERSONA','VEHICULO'=>'VEHICULO'], null, [
											'class'=>'form-control',
											'id'=>'m_caaplic',
											'placeholder'=>'APLICA',
											'title'=>"Lista de selección Aplica a"
										])
									!!}
									<label for="m_caaplic" class="glyphicon glyphicon-search" rel="tooltip" title="Seleccionar Aplica a">
									</label>
								</div>
							</div>
						</div>
						<!-- LISTA: CONDICION -->
						<div class="row form-group">
							<label class="col-sm-3 text-right control-label">Condición:</label>
						    <div class="col-md-9">
								<div class="icon-addon addon-md">
									{!! Form::select('m_cacondi', ['ACTIVO'=>'ACTIVO','INACTIVO'=>'INACTIVO','RETIRADO'=>'RETIRADO'], null, [
											'class'=>'form-control',
											'id'=>'m_cacondi',
											'placeholder'=>'CONDICION',
											'title'=>"Lista de selección Condición"
										])
									!!}
									<label for="m_cacondi" class="glyphicon glyphicon-search" rel="tooltip" title="Seleccionar Condición">
									</label>
								</div>
							</div>
						</div>
				...
				<div class="row">
					...
					<div class="col-md-2 text-right" style="padding-left: 9px">
						...
						@if(Auth::user()->m_capreep=="SI")
							<form action="{{ route('conceptos-pdf') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
								...
								<input type="hidden" name="m_cacondi" value="<?php if(isset($_GET['m_cacondi'])) echo $_GET['m_cacondi']; ?>">
								<input type="hidden" name="m_caapesv" value="<?php if(isset($_GET['m_caapesv'])) echo $_GET['m_caapesv']; ?>">
								<input type="hidden" name="m_cacodna" value="<?php if(isset($_GET['m_cacodna'])) echo $_GET['m_cacodna']; ?>">
								<input type="hidden" name="m_caestad" value="<?php if(isset($_GET['m_caestad'])) echo $_GET['m_caestad']; ?>">
								<input type="hidden" name="m_causreg" value="<?php if(isset($_GET['m_causreg'])) echo $_GET['m_causreg']; ?>">
					...
					<div class="col-md-3 text-left" style="padding-left: 25px">
						...
						@if(Auth::user()->m_capreex=="SI")
							<form action="{{ route('conceptos-excel') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
	...
	<script>
		...
		var anc_bar_pro = 0;
		var tot_reg_hal = {{$conceptos->total()}};
		var tot_reg_pen = {{$conceptos->total()}};

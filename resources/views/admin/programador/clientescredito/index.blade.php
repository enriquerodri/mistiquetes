...
//Descripción:      Página principal: Clientes crédito
//Actualizaciones
...
@extends('template')
@section('title','Clientes credito')
@include('sweet::alert')
...
@include('partials.menu-programador')
	<div class="container text-center">
		<!-- ADMINISTRATIVOS - CLIENTES CREDITO -->
		<hr class="separador">
    		<h3>CLIENTES CREDITO</h3>
    	<hr class="separador">
		...
		<div class="row text-center">
			...
			<div class="col-md-3">
				...
				<form action="{{ route('clientescredito.index') }}" method="get">
					<!-- ORDENAMIENTO -->
					<!-- ASCENDENTE - DESCENTE -->
					...
					<div class="col-md-5 text-left" style="padding-left: 0px">
						<button 
							...
							</i>&nbsp&nbsp&nbsp(&nbsp&nbspTotal Registros {{count($creditos)}} de {{$creditos->total()}}&nbsp&nbsp)
						</button>
					</div>
				</form>
			</div>
			<!-- PAGINADO SUPERIOR -->
			@if($creditos->total()>160)
					...
					<tr>
						<td>
							...
							@if(Auth::user()->m_capreed=="SI")
								<a onclick="consultar({{$credito->m_nucodig}});datosC({{$credito->m_nucociu}});" id="btneditar" class="btn btn-primary" data-toggle="modal" data-target="#modaleditar" title="Editar registro" data-placement="right"><i class="fa fa-edit" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>
								</a>
							@else
						...
						<td class="text-left">{{$credito->m_canuipe}}</td>
						<td>{{$credito->m_canombr}}</td>
						<td>{{$credito->m_cadirec}}</td>
						<td>{{$credito->ciudades->m_canombr}} - {{$departamento_x[$j]}} - {{$pais_x[$j]}}</td>
						<td>{{$credito->m_catelfi}}</td>
						<td>{{$credito->m_camovil}}</td>
						<td>{{$credito->m_cadacon}}</td>
						<td>{{$credito->m_cacodna}}</td>
						<td>{{$credito->m_caestad}}</td>
						<td>{{$credito->created_at}} </td>
						<td>{{$credito->usuario_registra->name}} </td>
						<td>{{$credito->updated_at}} </td>
						<td>{{$credito->usuario_actualiza->name}} </td>
					</tr>
					<?php $j++; ?>
				@endforeach
			</tbody>
			<!-- FIN CONTENIDO -->
			{{-- <td>{{$credito->departamentos->m_canombr}}</td>
			<td>{{$credito->paises->m_canombr}}</td> --}}
		</table>
		</div>
		<!-- FIN TABLA -->
		<!-- PAGINADO INFERIOR -->
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-check"></span>&nbsp&nbspCREAR CLIENTE CREDITO
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					...
					{!!Form::open(['route'=>'clientescredito.store','class'=>'form-horizontal']) !!}
					@include('admin.programador.clientescredito.form')
				</div>
				<!-- Pie -->
		...
		<div id="modaleditar" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-header" style="background-color: #3498db">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-edit"></span>&nbsp&nbspEDITANDO CLIENTE CREDITO
	    ...
	    function consultar(id){
	    	...
	    	$.ajax({
                ...
                success:function(data){
                    ...
                    $("#formeliminar").attr("action","clientescredito/"+id+"");
	...
	<div id="modalprocedimientos" class="modal fade">
	    ...
	    <div class="modal-content">
			...
			<div class="modal-header" style="background-color: #3498db">
				...
				<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-list-alt"></span>&nbsp&nbspPROCEDIMIENTOS CLIENTE CREDITO
				</h3>
			</div>
			<!-- Contenido -->
			<div class="modal-body">
		    	<div class="container-fluid">
					<object width="100%" height="50px" data="{{ asset('document/creditos.pdf') }}"></object>
		    	</div>
		    </div>
	    	<!-- Pie -->
			<div class="modal-footer">
		...
		<div class="modal-dialog modal-lg">
		    <div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-filter"></span>&nbsp&nbspFILTROS CLIENTE CREDITO
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					{!!Form::open(['route'=>'clientescredito.index','method'=>'get'])!!}
			    	<div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-6 text-right" style="padding-right: 0px">
						...
						@if(Auth::user()->m_capreep=="SI")
							<form action="{{ route('clientescredito-pdf') }}" method="get">
								<!-- ORDENAMIENTO -->
					...
					<div class="col-md-6 text-left" style="padding-left: 8px">
						...
						@if(Auth::user()->m_capreex=="SI")
							<form action="{{ route('clientescredito-excel') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
	...
	<script>
		...
		var anc_bar_pro = 0;
		var tot_reg_hal = {{$creditos->total()}};
		var tot_reg_pen = {{$creditos->total()}};

...
//Descripción:      Página principal: Capacitaciones
//Actualizaciones
...
@include('partials.menu-programador')
	<div class="container text-center">
		...
		<div class="row text-center">
			...
			<div class="col-md-3">
				...
				<form action="{{ route('capacitaciones.index') }}" method="get">
					<!-- ORDENAMIENTO -->
					...
					<div class="col-md-5 text-left" style="padding-left: 0px">
						<button 
							...
							</i>&nbsp&nbsp&nbsp(&nbsp&nbspTotal Registros {{count($capacitaciones)}} de {{$capacitaciones->total()}}&nbsp&nbsp)
						</button>
					</div>
				</form>
			</div>
			<!-- PAGINADO SUPERIOR -->
			@if($capacitaciones->total()>160)
				<div class="col-md-12 text-right">
			@else
				<div class="col-md-4 text-right">
			@endif
				{{$capacitaciones->appends([
					...
					<tr>
						<td>
							...
							@if(Auth::user()->m_capreed=="SI")
								<a onclick="consultar({{$capacitacion->m_nucodig}})" id="btneditar" class="btn btn-primary" data-toggle="modal" data-target="#modaleditar" title="Editar registro" data-placement="right"><i class="fa fa-edit" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>
								</a>
							@else
						...
						<td class="text-left">{{$capacitacion->m_canombr}}</td>
						<td>{{$capacitacion->m_caapesv}}</td>
						<td>{{$capacitacion->m_cacodna}}</td>
						<td>{{$capacitacion->m_caestad}}</td>
						<td>{{$capacitacion->created_at}}</td>
						<td>{{$capacitacion->usuario_registra->name}}</td>
						<td>{{$capacitacion->updated_at}}</td>
						<td>{{$capacitacion->usuario_actualiza->name}}</td>
					</tr>
				@endforeach
			</tbody>
			<!-- FIN CONTENIDO -->
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-check"></span>&nbsp&nbspCREAR CAPACITACION
					</h3>
				</div>
				<!-- Contenido -->
		...
		<div id="modaleditar" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-header" style="background-color: #3498db">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-edit"></span>&nbsp&nbspEDITANDO CAPACITACION
							...
							<div class="modal-footer">
								<div class="text-center">
									...
									@if(Auth::user()->m_capreel=="SI")
										<a onclick="eliminar({{$capacitacion->m_nucodig}})" id="btn btn-danger" class="btn btn-danger" data-toggle="modal" data-target="#modaleditar" title="" data-placement="right"><i class="fa fa-trash" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>&nbsp&nbspEliminar
										</a>
									@else
	    ...
	    function consultar(id){
	    	...
	    	$.ajax({
                ...
                success:function(data){
                    ...
                    $("#formeliminar").attr("action","capacitaciones/"+id+"");
                }               
            })     
	    }
	...
	<div id="modalprocedimientos" class="modal fade">
	    ...
	    <div class="modal-content">
			...
			<div class="modal-header" style="background-color: #3498db">
				...
				<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-list-alt"></span>&nbsp&nbspPROCEDIMIENTOS CAPACITACION
				</h3>
			</div>
			<!-- Contenido -->
			<div class="modal-body">
		    	<div class="container-fluid">
					<object width="100%" height="50px" data="{{ asset('document/capacitaciones.pdf') }}"></object>
		    	</div>
		    </div>
	    	<!-- Pie -->
			<div class="modal-footer">
	...
	<div id="modalfiltros" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
		    ...
		    <div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-filter"></span>&nbsp&nbspFILTROS CAPACITACION
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					{!!Form::open(['route'=>'capacitaciones.index','method'=>'get'])!!}
			    	<div class="container-fluid">
			    		<!-- CAJA DE TEXTO BUSQUEDA: NOMBRE -->	
			    		<div class="row form-group">
			    			<label class="col-sm-3 text-right control-label">Capacitación:</label>
			    			<div class="col-md-9">
			    				<div class="icon-addon addon-lg">
				    				{!! Form::text('m_canombr',null, [
										...
										'title'=>"Capacitación (5 a 50 caracteres - A-Z Ñ 0-9 #-_.&/)",
										'maxlength'=>'50',
				...
				<div class="row">
					...
					<div class="col-md-2 text-right" style="padding-left: 9px">
						...
						@if(Auth::user()->m_capreep=="SI")
							<form action="{{ route('capacitaciones-pdf') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
					...
					<div class="col-md-3 text-left" style="padding-left: 25px">
						...
						@if(Auth::user()->m_capreex=="SI")
							<form action="{{ route('capacitaciones-excel') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
	...
	<script>
		...
		var anc_bar_pro = 0;
		var tot_reg_hal = {{$capacitaciones->total()}};
		var tot_reg_pen = {{$capacitaciones->total()}};

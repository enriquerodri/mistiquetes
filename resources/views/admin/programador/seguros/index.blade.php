...
@include('partials.menu-programador')
				...
				<form action="{{ route('seguros.index') }}" method="get">
					...
					<div class="col-md-5 text-left" style="padding-left: 0px">
						<button 
							...
							</i>&nbsp&nbsp&nbsp(&nbsp&nbspTotal Registros {{count($seguros)}} de {{$seguros->total()}}&nbsp&nbsp)
						</button>
					</div>
				</form>
			</div>
			<!-- PAGINADO SUPERIOR -->
			@if($seguros->total()>160)
					...
					<tr>
						<td>
							...
							@if(Auth::user()->m_capreed=="SI")
								<a onclick="consultar({{$seguro->m_nucodig}})" id="btneditar" class="btn btn-primary" data-toggle="modal" data-target="#modaleditar" title="Editar registro" data-placement="right"><i class="fa fa-edit" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>
								</a>
							@else
						...
						<td class="text-left">{{$seguro->m_canombr}}</td>
						<td>{{$seguro->m_cadescr}}</td>
						<td>{{$seguro->m_caapper}}</td>
						<td>{{$seguro->m_caapvpa}}</td>
						<td>{{$seguro->m_caapvca}}</td>
						<td>{{$seguro->m_cacodna}}</td>
						<td>{{$seguro->m_caestad}}</td>
						<td>{{$seguro->created_at}}</td>
						<td>{{$seguro->usuario_registra->name}}</td>
						<td>{{$seguro->updated_at}}</td>
						<td>{{$seguro->usuario_actualiza->name}}</td>
					</tr>
				@endforeach
			</tbody>
			<!-- FIN CONTENIDO -->
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-check"></span>&nbsp&nbspCREAR SEGURO
					</h3>
				</div>
				<!-- Contenido -->
		...
		<div id="modaleditar" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-header" style="background-color: #3498db">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-edit"></span>&nbsp&nbspEDITANDO SEGURO
							...
							<div class="modal-footer">
								<div class="text-center">
									...
									@if(Auth::user()->m_capreel=="SI")
										<a onclick="eliminar({{$seguro->m_nucodig}})" id="btn btn-danger" class="btn btn-danger" data-toggle="modal" data-target="#modaleditar" title="" data-placement="right"><i class="fa fa-trash" style="font-size:18px;text-shadow:2px 2px 4px #000000;" ></i>&nbsp&nbspEliminar
										</a>
									@else
	    ...
	    function consultar(id){
	    	...
	    	$.ajax({
                ...
                success:function(data){
                    ...
                    console.log(data);
                    $("#formeditar").attr("action","seguros/"+id+"");
                    ...
                    $("#formeliminar").attr("action","seguros/"+id+"");
                }               
            })     
	    }
	...
	<div id="modalprocedimientos" class="modal fade">
	    ...
	    <div class="modal-content">
			...
			<div class="modal-header" style="background-color: #3498db">
				...
				<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-list-alt"></span>&nbsp&nbspPROCEDIMIENTOS SEGURO
				</h3>
			</div>
			<!-- Contenido -->
			<div class="modal-body">
		    	<div class="container-fluid">
					<object width="100%" height="50px" data="{{ asset('document/seguros.pdf') }}"></object>
		    	</div>
		    </div>
	    	<!-- Pie -->
			<div class="modal-footer">
	...
	<div id="modalfiltros" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
		    ...
		    <div class="modal-content">
				...
				<div class="modal-header" style="background-color: #3498db">
					...
					<h3 class="modal-title" id="myModalLabel" style="text-align: center;"><span class="glyphicon glyphicon-filter"></span>&nbsp&nbspFILTROS SEGURO
					</h3>
				</div>
				<!-- Contenido -->
				<div class="modal-body">
					{!!Form::open(['route'=>'seguros.index','method'=>'get'])!!}
			    	<div class="container-fluid">
			    		<!-- CAJA DE TEXTO BUSQUEDA: NOMBRE -->	
			    		<div class="row form-group">
			    			<label class="col-sm-3 text-right control-label">Seguro:</label>
			    			<div class="col-md-9">
						...
						<div class="form-group">
							<label class="col-sm-12 text-center control-label">ESTE SEGURO APLICA A</label>
						</div>
				...
				<div class="row">
					...
					<div class="col-md-2 text-right" style="padding-left: 9px">
						...
						@if(Auth::user()->m_capreep=="SI")
							<form action="{{ route('seguros-pdf') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
					...
					<div class="col-md-3 text-left" style="padding-left: 25px">
						...
						@if(Auth::user()->m_capreex=="SI")
							<form action="{{ route('seguros-excel') }}" method="get">
								<!-- ORDENAMIENTO -->
								<!-- ASCENDENTE - DESCENDENTE -->
								<input type="hidden" name="m_canombr_ordene"
												value="<?php if(isset($_GET['m_canombr_ordene']))
	...
	<script>
		...
		var anc_bar_pro = 0;
		var tot_reg_hal = {{$seguros->total()}};
		var tot_reg_pen = {{$seguros->total()}};

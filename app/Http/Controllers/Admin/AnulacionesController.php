...
//Nombre:           AnulacionesControler.php
//Descripción:      Motivos anulación documentos
//Actualizaciones
...
class AnulacionesController extends Controller
{
    ...
    private $m_caaplic_filtro="";
    private $m_caanure_filtro="";
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if(count($request->all())>0){
                ...
                $m_caaplic_filtro = $request->m_caaplic;
                $m_caanure_filtro = $request->m_caanure;
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $anulaciones = Capmanul::nombre($request->m_canombr)->
                    Codigo_nacional($request->m_cacodna)->
                    Aplicaa($request->m_caaplic)->
                    Anula_relacionados($request->m_caanure)->
                    estado($request->m_caestad)->
                    usuario_registra($request->m_causreg)->
                    usuario_actualiza($request->m_causact)->
                    orderBy('m_caaplic',$m_canombr_ordene)->
                    orderBy('m_caanure',$m_canombr_ordene)->
            ...
            //  VISTA
            return view('admin.programador.anulaciones.index',
                compact('anulaciones','usuarios','m_canombr_filtro',
                'm_cacodna_filtro','m_caaplic_filtro',
                'm_caanure_filtro','m_caestad_filtro',
    ...
    public function create(){
        return view('admin.programador.anulaciones.crear');
    }
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'Anulaciones';

        //2- CONSULTA SI EL NOMBRE YA EXISTE
        //$anulaciones = Capmanul::where('m_canombr',$request->m_canombr)->get();
        $anulaciones = Capmanul::where('m_canombr',$request->m_canombr)
        ->where('m_caaplic',$request->m_caaplic)
        ->where('m_caanure',$request->m_caanure)->get();

        if(count($anulaciones)>0){

            //2-1 EL NUEVO NOMBRE YA ESTA EN LA TABLA
            //2-2 RESULTADO ERROR: MENSAJES
            $tit_men_res_err = 'Motivo anulación: ';
            $tx1_men_res_err = 'Nombre: '.$request->m_canombr.' ';
            $tx2_men_res_err = 'Aplica a: '.$request->m_caaplic.' ';
            $tx3_men_res_err = 'Anular documentos relacionados: '.$request->m_caanure.' ';
            $tx4_men_res_err = '';
            $tx5_men_res_err = '';
            ...
            //return redirect()->route('anulaciones.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            return redirect()->route('anulaciones.index',
                    ['m_canombr'=>$request->m_canombr,
                    'm_caaplic'=>$request->m_caaplic,
                    'm_caanure'=>$request->m_caanure])
            ...
            $tit_men_res_cor = 'Motivo anulación: ';
            $tx1_men_res_cor = 'Nombre: '.$request->m_canombr.' ';
            $tx2_men_res_cor = 'Aplica a: '.$request->m_caaplic.' ';
            $tx3_men_res_cor = 'Anular documentos relacionados: '.$request->m_caanure.' ';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            return redirect()->route('anulaciones.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
        $anulacion = Capmanul::find($id);
        $causreg = User::find($anulacion->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
        $anulacion['nombre_causreg'] = $nombre_usuario_causreg;
    ...
    public function show($id){
        $anulacion = Capmanul::find($id);
        //return response()->json($anulacion);
        return view('admin.programador.anulaciones.consultar',compact('anulacion'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmanul::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if($anulacion->m_canombr==$request->m_canombr
            AND $anulacion->m_caaplic==$request->m_caaplic
            AND $anulacion->m_caanure==$request->m_caanure
            ){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $anulacion = Capmanul::find($id);
            $anulacion->m_canombr = $request->m_canombr;
            $anulacion->m_cacodna = $request->m_cacodna;
            $anulacion->m_caaplic = $request->m_caaplic;
            $anulacion->m_caestad = $request->m_caestad;
            $anulacion->m_caanure = $request->m_caanure;
            $anulacion->m_causact = Auth::user()->id;
            $anulacion->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmanul::find($id);
        ...
        } else if($anulacion->m_canombr!==$request->m_canombr
                OR $anulacion->m_caaplic!==$request->m_caaplic
                OR $anulacion->m_caanure!==$request->m_caanure){
            
            //2-3 EL NOMBRE FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NOMBRE NO ESTÉ YA EN LA TABLA
            $validar = Capmanul::where('m_canombr',$request->m_canombr)
            ->where('m_caaplic',$request->m_caaplic)
            ->where('m_caanure',$request->m_caanure)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($anulacion){
            ...
            $tit_men_res_cor = 'Motivo anulación: ';
            $tx1_men_res_cor = 'Nombre: '.$anulacion->m_canombr.' ';
            $tx2_men_res_cor = 'Aplica a: '.$anulacion->m_caaplic.' ';
            $tx3_men_res_cor = 'Anular documentos relacionados: '.$anulacion->m_caanure.' ';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            return redirect()->route('anulaciones.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        $this->m_caaplic_filtro=$request->m_caaplic;
        $this->m_caanure_filtro=$request->m_caanure;
        ...
        if($this->m_canombr_campos=="llave_compuesta"){
            $anulaciones = Capmanul::nombre($this->m_canombr_filtro)->
                            Codigo_nacional($this->m_cacodna_filtro)->
                            Aplicaa($this->m_caaplic_filtro)->
                            Anula_relacionados($this->m_caanure_filtro)->
                            estado($this->m_caestad_filtro)->
                            usuario_registra($this->m_causreg_filtro)->
                            usuario_actualiza($this->m_causact_filtro)->
                            orderBy('Capmanul.m_caaplic',$this->m_canombr_ordene)->
                            orderBy('Capmanul.m_caanure',$this->m_canombr_ordene)->
                            orderBy('Capmanul.m_canombr',$this->m_canombr_ordene)->get();
        }else{
            $anulaciones = Capmanul::nombre($this->m_canombr_filtro)->
                            Codigo_nacional($this->m_cacodna_filtro)->
                            Aplicaa($this->m_caaplic_filtro)->
        ...
        $nombre_archivo = 'anulaciones_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'anulaciones_'.$cadena_fecha_actual;

        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('Anulaciones', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA DE MOTIVOS ANULACIONES';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $anulaciones->Count();
                ...
                // ENCABEZADOS
                $sheet->row(7, [
                        'Identificador', 'Motivo', 'Aplica a',
                        'Anular documentos relacionados',
                ...
                // REGISTROS
                foreach($anulaciones as $index => $anulacion) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $anulacion->m_nucodig, $anulacion->m_canombr, $anulacion->m_caaplic,
                            $anulacion->m_caanure,
                            $anulacion->m_cacodna, $anulacion->m_caestad,
                            $anulacion->created_at, $anulacion->usuario_registra->name,
                            $anulacion->updated_at, $anulacion->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $anulacion->m_nucodig, $anulacion->m_canombr, $anulacion->m_caaplic,
                ...
                //$vista = \view('admin.programador.anulaciones.exportarexcel',compact('anulaciones'))->render();
                //$sheet->fromArray($anulaciones);

...
class CargosController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if(count($request->all())>0){
                ...
                $m_caticar_filtro = $request->m_caticar;
            ...
            //  CARGOS
            $cargos = Capmcarg::nombre($request->m_canombr)->
                tipo_cargo($request->m_caticar)->
                Division($request->m_nucodiv)->
            ...
            //  VISTA
            return view('admin.programador.cargos.index',
                compact('cargos','divisiones','usuarios','m_canombr_filtro',
                'm_caticar_filtro','m_nucodiv_filtro',
    ...
    public function create(){
        $cargos = Capmcarg::orderBy('m_canombr','asc')->pluck('m_canombr','m_nucodig');
        return view('admin.programador.cargos.crear',compact('cargos'));
    }
    ...
    public function store(Request $request){
        ...
        $cargos = Capmcarg::where('m_canombr',$request->m_canombr)
        ->where('m_nucodiv',$request->m_nucodiv)->get();

        //13-10-2018
        //2-1 LLAMAR A LA FUNCION QUE TRAE LOS DATOS DE CIUDAD - DEPARTAMENTO - PAIS
        $division = Capmdivi::where('m_nucodig',$request->m_nucodiv)->first();

        if(count($cargos)>0){

            //2-1 EL NUEVO NOMBRE YA ESTA EN LA TABLA
            //2-2 RESULTADO ERROR: MENSAJES
            $tit_men_res_err = 'Cargo: ';
            $tx1_men_res_err = ' '.$request->m_canombr.' ';
            $tx2_men_res_err = ' '.$division->m_canombr.' ';
            $tx3_men_res_err = '';
            $tx4_men_res_err = '';
            $tx5_men_res_err = '';
            ...
            $cargo = Capmcarg::create([
                'm_canombr'=>$request->m_canombr,
                'm_nucodiv'=>$request->m_nucodiv,
                'm_caticar'=>$request->m_caticar,
            ...
            $tit_men_res_cor = 'Cargo: ';
            $tx1_men_res_cor = ' '.$request->m_canombr.' ';
            $tx2_men_res_cor = ' '.$division->m_canombr.' ';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            return redirect()->route('cargos.index')->with($par_men_res_cor);
        }
    }
    ...
    public function show($id){
        $cargo = Capmcarg::find($id);
        $divisiones = Capmdivi::orderBy('m_canombr','asc')->pluck('m_canombr','m_nucodig');
        return view('admin.programador.cargos.consultar',compact('cargo','divisiones'));
    }
    ...
    public function update(Request $request, $id){
        ...
        if($cargo->m_canombr==$request->m_canombr
            AND $cargo->m_nucodiv==$request->m_nucodiv
            ){

            //2-3 CORRESPONDE AL MISMO NUIP (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $cargo = Capmcarg::find($id);
            $cargo->m_canombr = $request->m_canombr;
            $cargo->m_nucodiv = $request->m_nucodiv;
            $cargo->m_caticar = $request->m_caticar;
            $cargo->m_cacodna = $request->m_cacodna;
            $cargo->m_caestad = $request->m_caestad;
            $cargo->m_causact = Auth::user()->id;
            $cargo->save();

            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmcarg::find($id);
        ...
        } else if($cargo->m_canombr!==$request->m_canombr
                OR $cargo->m_nucodiv!==$request->m_nucodiv){
            
            //2-3 EL NOMBRE FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NOMBRE NO ESTÉ YA EN LA TABLA
            $validar = Capmcarg::where('m_canombr',$request->m_canombr)
            ->where('m_nucodiv',$request->m_nucodiv)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($cargo){
            ...
            $tit_men_res_cor = 'Cargo: ';
            $tx1_men_res_cor = ' '.$cargo->m_canombr.' ';
            $tx2_men_res_cor = ' '.$division->m_canombr.' ';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            return redirect()->route('cargos.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        $this->m_caticar_filtro=$request->m_caticar;
        $this->m_nucodiv_filtro=$request->m_nucodiv;
        ...
        $cargos = Capmcarg::nombre($this->m_canombr_filtro)->
                        tipo_cargo($this->m_caticar_filtro)->
                        Division($this->m_nucodiv_filtro)->
        ...
        //$vista = \view('admin.programador.cargos.exportarpdf_tiquete',compact('cargos'))->render();
        $vista = \view('admin.programador.cargos.exportarpdf',compact('cargos'))->render();
        $pdf->loadHTML($vista);
        ...
        $nombre_archivo = 'cargos_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'cargos_'.$cadena_fecha_actual;
        ...
        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('Cargos', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA DE CARGOS';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $cargos->Count();
                ...
                foreach($cargos as $index => $cargo) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $cargo->m_nucodig, $cargo->m_canombr,
                            $cargo->m_caticar, $cargo->division->m_canombr,
                            $cargo->m_cacodna, $cargo->m_caestad,
                            $cargo->created_at, $cargo->usuario_registra->name,
                            $cargo->updated_at, $cargo->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $cargo->m_nucodig, $cargo->m_canombr,

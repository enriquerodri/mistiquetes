...

{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            foreach($usuarios as $usuario){
                ...
                $ciudad_p = Capmciud::where('m_nucodig',$usuario->m_nucociu)->first();
                if(isset($ciudad_p)){
                    //dd($oficinas);
                    ...
                    //dd($usuarios);
                    $ciudad = Capmciud::where('m_nucodig','==',1)->first();
                    $departamento = Capmdepa::where('m_nucodig',$ciudad->m_nucodep)->first();
                    $pais = Capmpais::where('m_nucodig',$departamento->m_nucopai)->first();
                    $oficina_registro = Capmsucu::where('m_nucodig',$usuario->m_nuofici)->first();
            ...
            if(isset($usuario)){
                return view('admin.programador.usuarios.index',
                    compact('oficina','tiposdocumentos','ciudades','departamentos',
                            ...
                            'm_canuipe_filtro','email_filtro','m_canombr_filtro',
                            'm_nucociu_filtro',
                            'm_caestad_filtro',
                ...
                return view('admin.programador.usuarios.index',
                    compact('oficina','tiposdocumentos','ciudades','departamentos',
                            'paises','oficina_registro','ciudad_x','departamento_x','pais_x',
                            'usuarios','m_canuipe_filtro','email_filtro','m_canombr_filtro',
                            'm_nucociu_filtro',
                            'm_caestad_filtro',
                            'm_causreg_filtro', 'm_causact_filtro','usuariosRegAct',
                            'm_canombr_ordene','m_canombr_campos'));
    ...
    public function store(Request $request){
        ...
        $usuarios = Users::where('m_canuipe',$request->m_canuipe)->get();
        $validar_email = Users::where('email',$request->email)->get();
        //dd($usuarios);

        if ((count($usuarios)>0) || (count($validar_email)>1)){
        //if ((count($usuarios_mail)>0)){
            ...
            if ((count($usuarios)>0)) {
                ...
                $tx4_men_res_err = ' '.$request->email.' ';
                $fo1_men_res_err = 'Ya existe un registro con ese MAIL';
            }
            ...
            if ((count($usuarios)>0)) {
                ...
                return redirect()->route('usuarios.index',['email'=>$request->email])->with($par_men_res_err);
            }
        }
        else{
            //2-1 EL NUIP Y MAIL NUEVO  NO EXISTEN
            //$pass_cifrado = password_hash($request->password, PASSWORD_DEFAULT);
    ...
    public function update(Request $request, $id){
        ...
        if($usuario->m_canuipe==$request->m_canuipe){
            ...
            //2-3 EL NUIP NO FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL MAIL NO ESTÉ YA EN LA TABLA
            $validar_email = Users::where('email',$request->email)->get();
            if(count($validar_email)>1){
            ...
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NUIP Y EMAIL NO ESTEN YA EN LA TABLA
            $validar_usuar = Users::where('m_canuipe',$request->m_canuipe)->get();
            $validar_email = Users::where('email',$request->email)->get();
            //dd($usuarios);

            if ((count($validar_usuar)>0) || (count($validar_email)>1)){
            //if ((count($usuarios_mail)>0)){

                if ((count($validar_usuar)>0)) {
                    $tx4_men_res_err = '';
                    $fo1_men_res_err = 'Ya existe un registro con ese NUIP';
                } else {
                    $tx4_men_res_err = ' '.$request->email.' ';

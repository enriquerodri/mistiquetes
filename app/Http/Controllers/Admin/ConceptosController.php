...
class ConceptosController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if(count($request->all())>0){
                ...
                $m_cacondi_filtro = $request->m_cacondi;
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $conceptos = Capmsusp::nombre($request->m_canombr)->
                    Aplica($request->m_caaplic)->
                    Condicion($request->m_cacondi)->
            ...
            //  VISTA
            return view('admin.programador.conceptos.index',
                compact('conceptos','usuarios','m_canombr_filtro',
                'm_caaplic_filtro','m_cacondi_filtro',
    ...
    public function create(){
        return view('admin.programador.conceptos.crear');
    }
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'Conceptos';

        //2- CONSULTA SI EL NOMBRE YA EXISTE
        //$conceptos = Capmsusp::where('m_canombr',$request->m_canombr)->get();
        $conceptos = Capmsusp::where('m_canombr',$request->m_canombr)->get();
        ...
        if(count($conceptos)>0){
            ...
            //return redirect()->route('conceptos.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            ...
            return redirect()->route('conceptos.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
        $concepto = Capmsusp::find($id);
        $causreg = User::find($concepto->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
        $concepto['nombre_causreg'] = $nombre_usuario_causreg;
    ...
    public function show($id){
        $concepto = Capmsusp::find($id);
        //return response()->json($concepto);
        return view('admin.programador.conceptos.consultar',compact('concepto'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmsusp::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if($concepto->m_canombr==$request->m_canombr){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $concepto = Capmsusp::find($id);
            $concepto->m_canombr = $request->m_canombr;
            $concepto->m_caaplic = $request->m_caaplic;
            $concepto->m_cacondi = $request->m_cacondi;
            $concepto->m_cacodna = $request->m_cacodna;
            $concepto->m_caapesv = $request->m_caapesv;
            $concepto->m_caestad = $request->m_caestad;
            $concepto->m_causact = Auth::user()->id;
            $concepto->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmsusp::find($id);
        ...
        } else if($concepto->m_canombr!==$request->m_canombr){
            
            //2-3 EL NOMBRE FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NOMBRE NO ESTÉ YA EN LA TABLA
            $validar = Capmsusp::where('m_canombr',$request->m_canombr)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($concepto){
            ...
            $tit_men_res_cor = 'Concepto: ';
            $tx1_men_res_cor = 'Nombre: '.$concepto->m_canombr.' ';
            $tx2_men_res_cor = '';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            return redirect()->route('conceptos.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        $this->m_cacondi_filtro=$request->m_cacondi;
        ...
        if($this->m_canombr_campos=="llave_compuesta"){
            $conceptos = Capmsusp::nombre($this->m_canombr_filtro)->
                            Aplica($this->m_caaplic_filtro)->
                            Condicion($this->m_cacondi_filtro)->
                            ...
                            orderBy('Capmsusp.m_canombr',$this->m_canombr_ordene)->get();
        }else{
            $conceptos = Capmsusp::nombre($this->m_canombr_filtro)->
                            Aplica($this->m_caaplic_filtro)->
                            Condicion($this->m_cacondi_filtro)->
        ...
        $vista = \view('admin.programador.conceptos.exportarpdf',compact('conceptos'))->render();
        ...
        $nombre_archivo = 'Conceptos_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'conceptos_'.$cadena_fecha_actual;

        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('Conceptos', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA CONCEPTOS ACTIVACIONES - INACTIVACIONES';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $conceptos->Count();
                ...
                foreach($conceptos as $index => $concepto) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $concepto->m_nucodig, $concepto->m_canombr,
                            $concepto->m_caaplic, $concepto->m_cacondi,
                            $concepto->m_caapesv, $concepto->m_cacodna, $concepto->m_caestad,
                            $concepto->created_at, $concepto->usuario_registra->name,
                            $concepto->updated_at, $concepto->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $concepto->m_nucodig, $concepto->m_canombr,
                ...
                //$vista = \view('admin.programador.conceptos.exportarexcel',compact('conceptos'))->render();

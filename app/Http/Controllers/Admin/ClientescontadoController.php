...
//Nombre:           ClientescontadoControler.php
//Descripción:      Clientes facturación contado
//Actualizaciones
...
class ClientescontadoController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            $contados = Capmclgl::Nuipe($request->m_canuipe)->
            ...
            foreach($contados as $contado){
                $ciudad = Capmciud::where('m_nucodig',$contado->m_nucociu)->first();
                $departamento = Capmdepa::where('m_nucodig',$ciudad->m_nucodep)->first();
                $pais = Capmpais::where('m_nucodig',$departamento->m_nucopai)->first();
                $oficina_registro = Capmsucu::where('m_nucodig',$contado->m_nuofici)->first();
            ...
            return view('admin.programador.clientescontado.index',
                compact('contados','tiposdocumentos','ciudades','departamentos',
    ...
    public function create(){
        ...
        $usuarios = User::orderBy('name','asc')->pluck('name','id');
        //  VISTA
        return view('admin.programador.clientescontado.crear',
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'Clientes contado';

        //2- CONSULTA SI EL NUIP YA EXISTE
        $contados = Capmclgl::where('m_canuipe',$request->m_canuipe)->get();
        if(count($contados)>0){

            //2-1 EL NUEVO NOMBRE YA ESTA EN LA TABLA
            //2-2 RESULTADO ERROR: MENSAJES
            $tit_men_res_err = 'Cliente contado: ';
            ...
            return redirect()->route('clientescontado.index',['m_canuipe'=>$request->m_canuipe])->with($par_men_res_err);
        }
        else{
            
            //2-1 EL NUIP NUEVO NO EXISTE
            ...
            $tit_men_res_cor = 'Cliente contado: ';
            ...
            return redirect()->route('clientescontado.index')->with($par_men_res_cor);
        }
    }
    ...
    public function show($id){
        ...
        $oficina_registro = Capmsucu::orderBy('m_canombr','asc')->pluck('m_canombr','m_nucodig');
        //return response()->json($contado);
        return view('admin.programador.clientescontado.consultar',compact('contado','tiposdocumentos','ciudades','oficina_registro'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmclgl::find($id);

        //2-2 COMPARA NUIP (LLAVE) REGISTRO HALLADO CON NUIP (LLAVE) DE LA RESPUESTA
        if($contado->m_canuipe==$request->m_canuipe){

            //2-3 CORRESPONDE AL MISMO NUIP (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $contado = Capmclgl::find($id);
            $contado->m_canuipe = $request->m_canuipe;
            $contado->m_nudiver = $request->m_nudiver;
            $contado->m_nutipdo = $request->m_nutipdo;
            $contado->m_canombr = $request->m_canombr;
            $contado->m_caprnom = $request->m_caprnom;
            $contado->m_casenom = $request->m_casenom;
            $contado->m_caprape = $request->m_caprape;
            $contado->m_caseape = $request->m_caseape;
            $contado->m_cadirec = $request->m_cadirec;
            $contado->m_nucociu = $request->m_nucociu;
            $contado->m_catelfi = $request->m_catelfi;
            $contado->m_camovil = $request->m_camovil;
            $contado->m_casitew = $request->m_casitew;
            $contado->m_cacorel = $request->m_cacorel;
            $contado->m_cadacon = $request->m_cadacon;
            $contado->m_caautna = $request->m_caautna;
            $contado->m_cacodna = $request->m_cacodna;
            $contado->m_caestad = $request->m_caestad;
            $contado->m_fenacim = $request->m_fenacim;
            $contado->m_nuedada = $request->m_nuedada;
            $contado->m_catipsa = $request->m_catipsa;
            $contado->m_catisex = $request->m_catisex;
            $contado->m_nuofici = $request->m_nuofici;
            $contado->m_caviemp = $request->m_caviemp;
            $contado->m_causact = Auth::user()->id;
            $contado->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmclgl::find($id);
            ...
            if($con_dat_ini != $con_dat_fin){
                ...
                return redirect()->route('clientescontado.index');
            }

        } else if($contado->m_canuipe!==$request->m_canuipe) {
            
            //2-3 EL NUIP FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NUIP NO ESTÉ YA EN LA TABLA
            $validar = Capmclgl::where('m_canuipe',$request->m_canuipe)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($contado){
            ...
            $tit_men_res_cor = 'Cliente contado: ';
            $tx1_men_res_cor = ' '.$contado->m_canuipe.' ';
            $tx2_men_res_cor = ' '.$contado->m_canombr.' ';
            $tx3_men_res_cor = ' '.$contado->m_cacodna.' ';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            return redirect()->route('clientescontado.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        $contados = Capmclgl::Nuipe($this->m_canuipe_filtro)->
        ...
        $vista = \view('admin.programador.clientescontado.exportarpdf',compact('contados'))->render();
        $pdf->loadHTML($vista);
        ...
        $nombre_archivo = 'contados_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'contados_'.$cadena_fecha_actual;

        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('Contados', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA DE CLIENTES CONTADO';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $contados->Count();
                ...
                //13-10-2018
                $j=0;
                foreach($contados as $index => $contado) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $contado->m_nucodig,
                            $contado->m_canuipe, $contado->m_nudiver,
                            $contado->tiposdocumentos->m_canombr,
                            $contado->m_canombr,
                            $contado->m_caprnom, $contado->m_casenom,
                            $contado->m_caprape, $contado->m_caseape,
                            $contado->m_cadirec, $contado->ciudades->m_canombr,
                            $departamento_x[$j],
                            $pais_x[$j],
                            $contado->m_catelfi, $contado->m_camovil, $contado->m_casitew,
                            $contado->m_cacorel, $contado->m_cadacon, $contado->m_caautna,
                            $contado->m_cacodna, $contado->m_caestad,
                            $contado->m_caviemp, $contado->oficina_registro->m_canombr,
                            $contado->created_at, $contado->usuario_registra->name,
                            $contado->updated_at, $contado->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $contado->m_nucodig,
        ...
        return redirect()->route('contados.index');

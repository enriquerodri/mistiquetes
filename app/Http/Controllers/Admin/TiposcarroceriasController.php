...
//Nombre:           TiposcarroceriasControler.php
...
class TiposcarroceriasController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $tiposcarrocerias = Capmtica::nombre($request->m_canombr)->
            ...
            //  VISTA
            return view('admin.vehiculos.vehiculos.tipos-carrocerias.index',
                compact('tiposcarrocerias','usuarios','m_canombr_filtro',
    ...
    public function create(){
        return view('admin.vehiculos.vehiculos.tipos-carrocerias.crear');
    }
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'tipos-carrocerias';

        //2- CONSULTA SI EL NOMBRE YA EXISTE
        //$tiposcarrocerias = Capmtica::where('m_canombr',$request->m_canombr)->get();
        $tiposcarrocerias = Capmtica::where('m_canombr',$request->m_canombr)->get();

        if(count($tiposcarrocerias)>0){

            //2-1 EL NUEVO NOMBRE YA ESTA EN LA TABLA
            //2-2 RESULTADO ERROR: MENSAJES
            $tit_men_res_err = 'Tipo carroceria: ';
            ...
            //return redirect()->route('tipos-carrocerias.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            ...
            $tipocarroceria= Capmtica::create([
            ...
            $tit_men_res_cor = 'Tipo carroceria: ';
            ...
            return redirect()->route('tipos-carrocerias.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
        $tipocarroceria= Capmtica::find($id);
        $causreg = User::find($tipocarroceria->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
        $tipocarroceria['nombre_causreg'] = $nombre_usuario_causreg;
    ...
    public function show($id){
        $tipocarroceria= Capmtica::find($id);
        //return response()->json($tipocarroceria);
        return view('admin.vehiculos.vehiculos.tipos-carrocerias.consultar',compact('contrato'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmtica::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if($tipocarroceria->m_canombr==$request->m_canombr){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $tipocarroceria= Capmtica::find($id);
            $tipocarroceria->m_canombr = $request->m_canombr;
            $tipocarroceria->m_cacodna = $request->m_cacodna;
            $tipocarroceria->m_caestad = $request->m_caestad;
            $tipocarroceria->m_causact = Auth::user()->id;
            $tipocarroceria->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmtica::find($id);
        ...
        } else if($tipocarroceria->m_canombr!==$request->m_canombr){
            
            //2-3 EL NOMBRE FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NOMBRE NO ESTÉ YA EN LA TABLA
            $validar = Capmtica::where('m_canombr',$request->m_canombr)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($tipocarroceria){
            ...
            $tit_men_res_cor = 'Tipo carroceria: ';
            $tx1_men_res_cor = 'Nombre: '.$tipocarroceria->m_canombr.' ';
            $tx2_men_res_cor = '';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            $audi_borrar = Captaudi::create([
                'm_nuidope'=>$aud_idu_ope,
                'm_canombr'=>$aud_nom_pag,
                'm_callave'=>$tipocarroceria->m_canombr,
                'm_nuidusu'=>Auth::user()->id,
                'm_cadeini'=>$tipocarroceria]);

            //2-4 ELIMINA REGISTRO
            $tipocarroceria->delete();
            ...
            return redirect()->route('tipos-carrocerias.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        $vista = \view('admin.vehiculos.vehiculos.tipos-carrocerias.exportarpdf',compact('tiposcarrocerias'))->render();
        ...
        $nombre_archivo = 'tipos_carrocerias_'.$cadena_fecha_actual.'.pdf';
        //  Resultados

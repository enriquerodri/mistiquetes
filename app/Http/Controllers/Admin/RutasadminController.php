...
//Nombre:           RutasadminControler.php
...
class RutasadminController extends Controller
{
    ...
    private $m_nucocor_filtro="";
    private $m_nucocde_filtro="";
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if(count($request->all())>0){
                ...
                $m_nucocor_filtro = $request->m_nucocor;
                $m_nucocde_filtro = $request->m_nucocde;
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $rutas = Capmruta::nombre($request->m_canombr)->
                    Nombre_ciudad_origen($request->m_nucocor)->
                    Nombre_ciudad_destino($request->m_nucocde)->
            ...
            //  CIUDADES ORIGEN
            $ciudades_origen = DB::table('capmciud')->
            ...
            $departamentos_origen = Capmdepa::orderBy('m_canombr','asc')->pluck('m_canombr','m_nucodig','m_nucopai');
            //  PAISES ORIGEN
            $paises_origen = Capmpais::orderBy('m_canombr','asc')->pluck('m_canombr','m_nucodig');

            //  CIUDADS DESTINO 
            $ciudades_destino = DB::table('capmciud')->
                            join('capmdepa','capmdepa.m_nucodig','=','capmciud.m_nucodep')->
                            join('capmpais','capmpais.m_nucodig','=','capmdepa.m_nucopai')->
                            selectRaw('CONCAT(capmciud.m_canombr, " - ", capmdepa.m_canombr, " - ", capmpais.m_canombr) as m_nucociu, capmciud.m_nucodig')->
                            orderBy('capmciud.m_canombr','asc')->
            ...
            $departamentos_destino = Capmdepa::orderBy('m_canombr','asc')->pluck('m_canombr','m_nucodig','m_nucopai');
            //  PAISES DESTINO
            $paises_destino = Capmpais::orderBy('m_canombr','asc')->pluck('m_canombr','m_nucodig');

            $usuarios = User::orderBy('name','asc')->pluck('name','id');
            //  VISTA
            return view('admin.vehiculos.rutas.registro.index',
                compact('rutas','ciudades_origen','departamentos_origen','paises_origen',
                'ciudades_destino','departamentos_destino','paises_destino',
                'usuarios','m_canombr_filtro',
    ...
    public function create(){
        ...
        return view('admin.vehiculos.rutas.crear',compact('ciudades'));
    }
    ...
    public function store(Request $request){
        ...
        //$rutas = Capmruta::where('m_canombr',$request->m_canombr)->get();
        $rutas = Capmruta::where('m_canombr',$request->m_canombr)->get();
        ...
        if(count($rutas)>0){
            ...
            //return redirect()->route('rutas.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            ...
            return redirect()->route('rutas.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
        $ruta= Capmruta::find($id);
        $ciudades_origen = Capmciud::orderBy('m_canombr','asc')->pluck('m_canombr','m_nucodig');
        $ciudades_destino = Capmciud::orderBy('m_canombr','asc')->pluck('m_canombr','m_nucodig');
        $causreg = User::find($ruta->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
        $ruta['nombre_causreg'] = $nombre_usuario_causreg;
    ...
    public function show($id){
        ...
        $ciudades = Capmciud::orderBy('m_canombr','asc')->pluck('m_canombr','m_nucodig');
        //return response()->json($ruta);
        return view('admin.vehiculos.rutas.consultar',compact('ruta','ciudades'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmruta::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if($ruta->m_canombr==$request->m_canombr){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $ruta= Capmruta::find($id);
            $ruta->m_canombr = $request->m_canombr;
            $ruta->m_camapas = $request->m_camapas;
            $ruta->m_nucoori = $request->m_nucoori;
            $ruta->m_nucodes = $request->m_nucodes;
            $ruta->m_cacondu = $request->m_cacondu;
            $ruta->m_nukilom = $request->m_nukilom;
            $ruta->m_catiemp = $request->m_catiemp;
            $ruta->m_catipos = $request->m_catipos;
            $ruta->m_caclase = $request->m_caclase;
            $ruta->m_caresol = $request->m_caresol;
            $ruta->m_feresol = $request->m_feresol;
            $ruta->m_cacodna = $request->m_cacodna;
            $ruta->m_caestad = $request->m_caestad;
            $ruta->m_causact = Auth::user()->id;
            $ruta->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmruta::find($id);
    ...
    public function destroy($id){
        ...
        if($ruta){
            ...
            $tit_men_res_cor = 'Ruta: ';
            $tx1_men_res_cor = 'Nombre: '.$ruta->m_canombr.' ';
            $tx2_men_res_cor = '';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            return redirect()->route('rutas.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        if($this->m_canombr_campos=="llave_compuesta"){
            $rutas = Capmruta::nombre($this->m_canombr_filtro)->
                            ...
                            orderBy(' Capmruta.m_canombr',$this->m_canombr_ordene)->get();
        }else{
            $rutas = Capmruta::nombre($this->m_canombr_filtro)->
                            Codigo_nacional($this->m_cacodna_filtro)->
                            estado($this->m_caestad_filtro)->
        ...
        $vista = \view('admin.vehiculos.rutas.exportarpdf',compact('rutas'))->render();
        ...
        $nombre_archivo = 'rutas_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'rutas_'.$cadena_fecha_actual;

        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('rutas', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA RUTAS';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $rutas->Count();
                ...
                foreach($rutas as $index => $ruta) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $ruta->m_nucodig, $ruta->m_canombr,
                            $ruta->m_cacodna, $ruta->m_caestad,
                            $ruta->created_at, $ruta->usuario_registra->name,
                            $ruta->updated_at, $ruta->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $ruta->m_nucodig, $ruta->m_canombr,

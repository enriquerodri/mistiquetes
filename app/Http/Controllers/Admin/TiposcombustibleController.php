...
//Nombre:           TiposcombustibleControler.php
//Descripción:      Tipos de combustible para clasificación vehículos
//Actualizaciones
...
class TiposcombustibleController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $tiposcombustibles = Capmclco::nombre($request->m_canombr)->
            ...
            //  VISTA
            return view('admin.vehiculos.vehiculos.tipos-combustible.index',
                compact('tiposcombustibles','usuarios','m_canombr_filtro',
    ...
    public function create(){
        return view('admin.vehiculos.vehiculos.tipos-combustible.crear');
    }
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'tipos-combustible';

        //2- CONSULTA SI EL NOMBRE YA EXISTE
        //$tiposcombustibles = Capmclco::where('m_canombr',$request->m_canombr)->get();
        $tiposcombustibles = Capmclco::where('m_canombr',$request->m_canombr)->get();

        if(count($tiposcombustibles)>0){

            //2-1 EL NUEVO NOMBRE YA ESTA EN LA TABLA
            //2-2 RESULTADO ERROR: MENSAJES
            $tit_men_res_err = 'Tipo combustible: ';
            ...
            //return redirect()->route('tipos-combustible.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            ...
            $tipocombustible= Capmclco::create([
            ...
            $tit_men_res_cor = 'Tipo combustible: ';
            ...
            return redirect()->route('tipos-combustible.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
        $tipocombustible= Capmclco::find($id);
        $causreg = User::find($tipocombustible->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
        $tipocombustible['nombre_causreg'] = $nombre_usuario_causreg;
    ...
    public function show($id){
        $tipocombustible= Capmclco::find($id);
        //return response()->json($tipocombustible);
        return view('admin.vehiculos.vehiculos.tipos-combustible.consultar',compact('contrato'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmclco::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if($tipocombustible->m_canombr==$request->m_canombr){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $tipocombustible= Capmclco::find($id);
            $tipocombustible->m_canombr = $request->m_canombr;
            $tipocombustible->m_cacodna = $request->m_cacodna;
            $tipocombustible->m_caestad = $request->m_caestad;
            $tipocombustible->m_causact = Auth::user()->id;
            $tipocombustible->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmclco::find($id);
        ...
        } else if($tipocombustible->m_canombr!==$request->m_canombr){
            
            //2-3 EL NOMBRE FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NOMBRE NO ESTÉ YA EN LA TABLA
            $validar = Capmclco::where('m_canombr',$request->m_canombr)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($tipocombustible){
            ...
            $tit_men_res_cor = 'Tipo combustible: ';
            $tx1_men_res_cor = 'Nombre: '.$tipocombustible->m_canombr.' ';
            $tx2_men_res_cor = '';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            $audi_borrar = Captaudi::create([
                'm_nuidope'=>$aud_idu_ope,
                'm_canombr'=>$aud_nom_pag,
                'm_callave'=>$tipocombustible->m_canombr,
                'm_nuidusu'=>Auth::user()->id,
                'm_cadeini'=>$tipocombustible]);
            ...
            return redirect()->route('tipos-combustible.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        if($this->m_canombr_campos=="llave_compuesta"){
            $tiposcombustibles = Capmclco::nombre($this->m_canombr_filtro)->
                            ...
                            orderBy(' Capmclco.m_canombr',$this->m_canombr_ordene)->get();
        }else{
            $tiposcombustibles = Capmclco::nombre($this->m_canombr_filtro)->
                            Codigo_nacional($this->m_cacodna_filtro)->
                            estado($this->m_caestad_filtro)->
        ...
        $vista = \view('admin.vehiculos.vehiculos.tipos-combustible.exportarpdf',compact('tiposcombustibles'))->render();
        ...
        $nombre_archivo = 'tipos_combustible_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'tipos_combustible_'.$cadena_fecha_actual;

        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('tipos_combustible', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA TIPOS COMBUSTIBLE VEHICULO';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $tiposcombustibles->Count();
                ...
                // ENCABEZADOS
                $sheet->row(7, [
                        'Identificador', 'Tipo combustible vehículo',
                ...
                // REGISTROS
                foreach($tiposcombustibles as $index => $tipocombustible) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $tipocombustible->m_nucodig, $tipocombustible->m_canombr,
                            $tipocombustible->m_cacodna, $tipocombustible->m_caestad,
                            $tipocombustible->created_at, $tipocombustible->usuario_registra->name,
                            $tipocombustible->updated_at, $tipocombustible->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $tipocombustible->m_nucodig, $tipocombustible->m_canombr,

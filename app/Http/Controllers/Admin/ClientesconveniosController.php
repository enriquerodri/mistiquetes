...
//Nombre:           ClientesconveniosControler.php
//Descripción:      Clientes convenios empresariales
//Actualizaciones
...
class ClientesconveniosController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            //  CLIENTES CONVENIOS
            $convenios = Capmclcv::Nuipe($request->m_canuipe)->
            ...
            foreach($convenios as $convenio){
                $ciudad = Capmciud::where('m_nucodig',$convenio->m_nucociu)->first();
            ...
            //  VISTA
            return view('admin.programador.clientesconvenios.index',
                compact('convenios','tiposdocumentos','ciudades','departamentos',
    ...
    public function create(){
        ...
        $usuarios = User::orderBy('name','asc')->pluck('name','id');
        //  VISTA
        return view('admin.programador.clientesconvenios.crear',
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'Clientes convenios';

        //2- CONSULTA SI EL NUIP YA EXISTE
        $convenios = Capmclcv::where('m_canuipe',$request->m_canuipe)->get();
        if(count($convenios)>0){

            //2-1 EL NUEVO NOMBRE YA ESTA EN LA TABLA
            //2-2 RESULTADO ERROR: MENSAJES
            $tit_men_res_err = 'Cliente convenio: ';
            ...
            return redirect()->route('clientesconvenios.index',['m_canuipe'=>$request->m_canuipe])->with($par_men_res_err);
        }
        else{
            
            //2-1 EL NOMBRE NUEVO NO EXISTE
            //2-2 INSERTAR LOS DATOS
            $convenio = Capmclcv::create([
            ...
            $tit_men_res_cor = 'Cliente convenio: ';
            ...
            return redirect()->route('clientesconvenios.index')->with($par_men_res_cor);
        }
    }
    ...
    public function show($id){
        ...
        $ciudades = Capmciud::orderBy('m_canombr','asc')->pluck('m_canombr','m_nucodig');
        //return response()->json($convenio);
        return view('admin.programador.clientesconvenios.consultar',compact('convenio','tiposdocumentos','ciudades'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmclcv::find($id);

        //2-2 COMPARA NUIP (LLAVE) REGISTRO HALLADO CON NUIP (LLAVE) DE LA RESPUESTA
        if($convenio->m_canuipe==$request->m_canuipe){

            //2-3 CORRESPONDE AL MISMO NUIP (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $convenio = Capmclcv::find($id);
            $convenio->m_canuipe = $request->m_canuipe;
            $convenio->m_nudiver = $request->m_nudiver;
            $convenio->m_nutipdo = $request->m_nutipdo;
            $convenio->m_canombr = $request->m_canombr;
            $convenio->m_caprnom = $request->m_caprnom;
            $convenio->m_casenom = $request->m_casenom;
            $convenio->m_caprape = $request->m_caprape;
            $convenio->m_caseape = $request->m_caseape;
            $convenio->m_cadirec = $request->m_cadirec;
            $convenio->m_nucociu = $request->m_nucociu;
            $convenio->m_catelfi = $request->m_catelfi;
            $convenio->m_camovil = $request->m_camovil;
            $convenio->m_casitew = $request->m_casitew;
            $convenio->m_cacorel = $request->m_cacorel;
            $convenio->m_cadacon = $request->m_cadacon;
            $convenio->m_caautna = $request->m_caautna;
            $convenio->m_cacodna = $request->m_cacodna;
            $convenio->m_caestad = $request->m_caestad;
            $convenio->m_causact = Auth::user()->id;
            $convenio->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmclcv::find($id);
            ...
            if($con_dat_ini != $con_dat_fin){
                ...
                return redirect()->route('clientesconvenios.index');
            }

        } else if($convenio->m_canuipe!==$request->m_canuipe) {
            
            //2-3 EL NUIP FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NUIP NO ESTÉ YA EN LA TABLA
            $validar = Capmclcv::where('m_canuipe',$request->m_canuipe)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($convenio){
            ...
            $tit_men_res_cor = 'Cliente convenio: ';
            $tx1_men_res_cor = ' '.$convenio->m_canuipe.' ';
            $tx2_men_res_cor = ' '.$convenio->m_canombr.' ';
            $tx3_men_res_cor = ' '.$convenio->m_cacodna.' ';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            return redirect()->route('clientesconvenios.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        $convenios = Capmclcv::Nuipe($this->m_canuipe_filtro)->
        ...
        $vista = \view('admin.programador.clientesconvenios.exportarpdf',compact('convenios'))->render();
        $pdf->loadHTML($vista);
        ...
        $nombre_archivo = 'convenios_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'convenios_'.$cadena_fecha_actual;
        ...
        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('Convenios', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA DE CLIENTES CONVENIOS';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $convenios->Count();
                ...
                $sheet->row(7, [
                        ...
                        'Tipo de documento','Cliente convenio',
                ...
                foreach($convenios as $index => $convenio) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $convenio->m_nucodig,
                            $convenio->m_canuipe, $convenio->m_nudiver,
                            $convenio->tiposdocumentos->m_canombr,
                            $convenio->m_canombr,
                            $convenio->m_caprnom, $convenio->m_casenom,
                            $convenio->m_caprape, $convenio->m_caseape,
                            $convenio->m_cadirec, $convenio->ciudades->m_canombr,
                            $departamento_x[$j],
                            $pais_x[$j],
                            $convenio->m_catelfi, $convenio->m_camovil, $convenio->m_casitew,
                            $convenio->m_cacorel, $convenio->m_cadacon, $convenio->m_caautna,
                            $convenio->m_cacodna, $convenio->m_caestad,
                            $convenio->created_at, $convenio->usuario_registra->name,
                            $convenio->updated_at, $convenio->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $convenio->m_nucodig,

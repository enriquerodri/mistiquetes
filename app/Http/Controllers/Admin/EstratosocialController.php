...
//Nombre:           EstratosocialControler.php
//Descripción:      Estrato social (humanos)
//Actualizaciones
...
class EstratosocialController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $estratossociales = Capmesso::nombre($request->m_canombr)->
            ...
            //  VISTA
            return view('admin.programador.estratosocial.index',
                compact('estratossociales','usuarios','m_canombr_filtro',
    ...
    public function create(){
        return view('admin.programador.estratosocial.crear');
    }
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'estratosocial';

        //2- CONSULTA SI EL NOMBRE YA EXISTE
        //$estratossociales = Capmesso::where('m_canombr',$request->m_canombr)->get();
        $estratossociales = Capmesso::where('m_canombr',$request->m_canombr)->get();

        if(count($estratossociales)>0){

            //2-1 EL NUEVO NOMBRE YA ESTA EN LA TABLA
            //2-2 RESULTADO ERROR: MENSAJES
            $tit_men_res_err = 'Estrato social: ';
            ...
            //return redirect()->route('estratosocial.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            return redirect()->route('estratosocial.index',
            ...
            $tit_men_res_cor = 'Estrato social: ';
            ...
            return redirect()->route('estratosocial.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
        $estratosocial= Capmesso::find($id);
        $causreg = User::find($estratosocial->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
        $estratosocial['nombre_causreg'] = $nombre_usuario_causreg;

        //2-1 RESULTADOS: ENVIA
        return response()->json($estratosocial);
    }
    ...
    public function show($id){
        $estratosocial= Capmesso::find($id);
        //return response()->json($estratosocial);
        return view('admin.programador.estratosocial.consultar',compact('contrato'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmesso::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if($estratosocial->m_canombr==$request->m_canombr){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $estratosocial= Capmesso::find($id);
            $estratosocial->m_canombr = $request->m_canombr;
            $estratosocial->m_cacodna = $request->m_cacodna;
            $estratosocial->m_caestad = $request->m_caestad;
            $estratosocial->m_causact = Auth::user()->id;
            $estratosocial->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmesso::find($id);
        ...
        } else if($estratosocial->m_canombr!==$request->m_canombr){
            
            //2-3 EL NOMBRE FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NOMBRE NO ESTÉ YA EN LA TABLA
            $validar = Capmesso::where('m_canombr',$request->m_canombr)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($estratosocial){
            ...
            $tit_men_res_cor = 'Estrato social: ';
            $tx1_men_res_cor = 'Nombre: '.$estratosocial->m_canombr.' ';
            $tx2_men_res_cor = '';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            $audi_borrar = Captaudi::create([
                'm_nuidope'=>$aud_idu_ope,
                'm_canombr'=>$aud_nom_pag,
                'm_callave'=>$estratosocial->m_canombr,
                'm_nuidusu'=>Auth::user()->id,
                'm_cadeini'=>$estratosocial]);

            //2-4 ELIMINA REGISTRO
            $estratosocial->delete();
            ...
            return redirect()->route('estratosocial.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        if($this->m_canombr_campos=="llave_compuesta"){
            $estratossociales = Capmesso::nombre($this->m_canombr_filtro)->
                            ...
                            orderBy(' Capmesso.m_canombr',$this->m_canombr_ordene)->get();
        }else{
            $estratossociales = Capmesso::nombre($this->m_canombr_filtro)->
                            Codigo_nacional($this->m_cacodna_filtro)->
                            estado($this->m_caestad_filtro)->
        ...
        $vista = \view('admin.programador.estratosocial.exportarpdf',compact('estratossociales'))->render();
        ...
        $nombre_archivo = 'estratosocial_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'estratosocial_'.$cadena_fecha_actual;

        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('estratosocial', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA ESTRATO SOCIAL';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $estratossociales->Count();
                ...
                // REGISTROS
                foreach($estratossociales as $index => $estratosocial) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $estratosocial->m_nucodig, $estratosocial->m_canombr,
                            $estratosocial->m_cacodna, $estratosocial->m_caestad,
                            $estratosocial->created_at, $estratosocial->usuario_registra->name,
                            $estratosocial->updated_at, $estratosocial->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $estratosocial->m_nucodig, $estratosocial->m_canombr,

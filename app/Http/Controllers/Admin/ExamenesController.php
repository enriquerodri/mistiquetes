...
//Nombre:           ExamenesControler.php
//Descripción:      Examenes médicos humanos vinculados a la organización
//Actualizaciones
...
class ExamenesController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $examenes = Capmexme::nombre($request->m_canombr)->
            ...
            //  VISTA
            return view('admin.programador.examenes.index',
                compact('examenes','usuarios','m_canombr_filtro',
    ...
    public function create(){
        return view('admin.programador.examenes.crear');
    }
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'examenes';

        //2- CONSULTA SI EL NOMBRE YA EXISTE
        //$examenes = Capmexme::where('m_canombr',$request->m_canombr)->get();
        $examenes = Capmexme::where('m_canombr',$request->m_canombr)->get();
        ...
        if(count($examenes)>0){
            ...
            //return redirect()->route('examenes.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            ...
            return redirect()->route('examenes.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
         $examen = Capmexme::find($id);
        $causreg = User::find( $examen->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
         $examen['nombre_causreg'] = $nombre_usuario_causreg;
    ...
    public function show($id){
         $examen = Capmexme::find($id);
        //return response()->json( $examen);
        return view('admin.programador.examenes.consultar',compact('contrato'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmexme::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if( $examen->m_canombr==$request->m_canombr){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
             $examen = Capmexme::find($id);
             $examen->m_canombr = $request->m_canombr;
             $examen->m_cacodna = $request->m_cacodna;
             $examen->m_caapesv = $request->m_caapesv;
             $examen->m_caestad = $request->m_caestad;
             $examen->m_causact = Auth::user()->id;
             $examen->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmexme::find($id);
        ...
        } else if( $examen->m_canombr!==$request->m_canombr){
            
            //2-3 EL NOMBRE FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NOMBRE NO ESTÉ YA EN LA TABLA
            $validar = Capmexme::where('m_canombr',$request->m_canombr)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if( $examen){
            ...
            $tit_men_res_cor =  'Examen: ';
            $tx1_men_res_cor = 'Nombre: '. $examen->m_canombr.' ';
            $tx2_men_res_cor = '';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            return redirect()->route('examenes.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        if($this->m_canombr_campos=="llave_compuesta"){
            $examenes = Capmexme::nombre($this->m_canombr_filtro)->
                            ...
                            orderBy('Capmexme.m_canombr',$this->m_canombr_ordene)->get();
        }else{
            $examenes = Capmexme::nombre($this->m_canombr_filtro)->
                            Codigo_nacional($this->m_cacodna_filtro)->
                            Pesv($this->m_caapesv_filtro)->
        ...
        $vista = \view('admin.programador.examenes.exportarpdf',compact('examenes'))->render();
        ...
        $nombre_archivo = 'examenes_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'examenes_'.$cadena_fecha_actual;

        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('examenes', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA EXAMENES';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $examenes->Count();
                ...
                foreach($examenes as $index =>  $examen) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                             $examen->m_nucodig,  $examen->m_canombr,
                             $examen->m_caapesv,  $examen->m_cacodna,  $examen->m_caestad,
                             $examen->created_at,  $examen->usuario_registra->name,
                             $examen->updated_at,  $examen->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                             $examen->m_nucodig,  $examen->m_canombr,
                ...
                //$vista = \view('admin.programador.examenes.exportarexcel',compact('examenes'))->render();
                //$sheet->fromArray($examenes);

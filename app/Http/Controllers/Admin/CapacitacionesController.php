...
class CapacitacionesController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $capacitaciones = Capmcapa::nombre($request->m_canombr)->
            ...
            //  VISTA
            return view('admin.programador.capacitaciones.index',
                compact('capacitaciones','usuarios','m_canombr_filtro',
    ...
    public function create(){
        return view('admin.programador.capacitaciones.crear');
    }
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'Capacitaciones';

        //2- CONSULTA SI EL NOMBRE YA EXISTE
        //$capacitaciones = Capmcapa::where('m_canombr',$request->m_canombr)->get();
        $capacitaciones = Capmcapa::where('m_canombr',$request->m_canombr)->get();
        ...
        if(count($capacitaciones)>0){
            ...
            //return redirect()->route('capacitaciones.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            ...
            return redirect()->route('capacitaciones.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
        $capacitacion = Capmcapa::find($id);
        $causreg = User::find($capacitacion->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
        $capacitacion['nombre_causreg'] = $nombre_usuario_causreg;
    ...
    public function show($id){
        $capacitacion = Capmcapa::find($id);
        //return response()->json($capacitacion);
        return view('admin.programador.capacitaciones.consultar',compact('contrato'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmcapa::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if($capacitacion->m_canombr==$request->m_canombr){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $capacitacion = Capmcapa::find($id);
            $capacitacion->m_canombr = $request->m_canombr;
            $capacitacion->m_cacodna = $request->m_cacodna;
            $capacitacion->m_caapesv = $request->m_caapesv;
            $capacitacion->m_caestad = $request->m_caestad;
            $capacitacion->m_causact = Auth::user()->id;
            $capacitacion->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmcapa::find($id);
        ...
        } else if($capacitacion->m_canombr!==$request->m_canombr){
            
            //2-3 EL NOMBRE FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NOMBRE NO ESTÉ YA EN LA TABLA
            $validar = Capmcapa::where('m_canombr',$request->m_canombr)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($capacitacion){
            ...
            $tit_men_res_cor = 'Capacitación: ';
            $tx1_men_res_cor = 'Nombre: '.$capacitacion->m_canombr.' ';
            $tx2_men_res_cor = '';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            $audi_borrar = Captaudi::create([
                'm_nuidope'=>$aud_idu_ope,
                'm_canombr'=>$aud_nom_pag,
                'm_callave'=>$capacitacion->m_canombr,
                'm_nuidusu'=>Auth::user()->id,
                'm_cadeini'=>$capacitacion]);
            ...
            return redirect()->route('capacitaciones.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        if($this->m_canombr_campos=="llave_compuesta"){
            $capacitaciones = Capmcapa::nombre($this->m_canombr_filtro)->
                            ...
                            orderBy('Capmcapa.m_canombr',$this->m_canombr_ordene)->get();
        }else{
            $capacitaciones = Capmcapa::nombre($this->m_canombr_filtro)->
                            Codigo_nacional($this->m_cacodna_filtro)->
                            Pesv($this->m_caapesv_filtro)->
        ...
        $vista = \view('admin.programador.capacitaciones.exportarpdf',compact('capacitaciones'))->render();
        ...
        $nombre_archivo = 'capacitaciones_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'capacitaciones_'.$cadena_fecha_actual;

        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('Capacitaciones', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA CAPACITACIONES';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $capacitaciones->Count();
                ...
                foreach($capacitaciones as $index => $capacitacion) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $capacitacion->m_nucodig, $capacitacion->m_canombr,
                            $capacitacion->m_caapesv, $capacitacion->m_cacodna, $capacitacion->m_caestad,
                            $capacitacion->created_at, $capacitacion->usuario_registra->name,
                            $capacitacion->updated_at, $capacitacion->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $capacitacion->m_nucodig, $capacitacion->m_canombr,
                ...
                //$vista = \view('admin.programador.capacitaciones.exportarexcel',compact('capacitaciones'))->render();
                //$sheet->fromArray($capacitaciones);

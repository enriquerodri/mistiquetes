...
//Nombre:           EstadocivilControler.php
...
class EstadocivilController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $estadosciviles =  Capmesci::nombre($request->m_canombr)->
            ...
            //  VISTA
            return view('admin.programador.estadocivil.index',
                compact('estadosciviles','usuarios','m_canombr_filtro',
    ...
    public function create(){
        return view('admin.programador.estadocivil.crear');
    }
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'estadosciviles';

        //2- CONSULTA SI EL NOMBRE YA EXISTE
        //$estadosciviles =  Capmesci::where('m_canombr',$request->m_canombr)->get();
        $estadosciviles =  Capmesci::where('m_canombr',$request->m_canombr)->get();
        ...
        if(count($estadosciviles)>0){
            ...
            //return redirect()->route('estadocivil.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            ...
            return redirect()->route('estadocivil.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
        $estadocivil=  Capmesci::find($id);
        $causreg = User::find($estadocivil->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
        $estadocivil['nombre_causreg'] = $nombre_usuario_causreg;
    ...
    public function show($id){
        $estadocivil=  Capmesci::find($id);
        //return response()->json($estadocivil);
        return view('admin.programador.estadocivil.consultar',compact('contrato'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini =  Capmesci::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if($estadocivil->m_canombr==$request->m_canombr){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $estadocivil=  Capmesci::find($id);
            $estadocivil->m_canombr = $request->m_canombr;
            $estadocivil->m_cacodna = $request->m_cacodna;
            $estadocivil->m_caestad = $request->m_caestad;
            $estadocivil->m_causact = Auth::user()->id;
            $estadocivil->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin =  Capmesci::find($id);
        ...
        } else if($estadocivil->m_canombr!==$request->m_canombr){
            
            //2-3 EL NOMBRE FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NOMBRE NO ESTÉ YA EN LA TABLA
            $validar =  Capmesci::where('m_canombr',$request->m_canombr)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($estadocivil){
            ...
            $tit_men_res_cor = 'Estado civil: ';
            $tx1_men_res_cor = 'Nombre: '.$estadocivil->m_canombr.' ';
            $tx2_men_res_cor = '';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            $audi_borrar = Captaudi::create([
                'm_nuidope'=>$aud_idu_ope,
                'm_canombr'=>$aud_nom_pag,
                'm_callave'=>$estadocivil->m_canombr,
                'm_nuidusu'=>Auth::user()->id,
                'm_cadeini'=>$estadocivil]);
            ...
            return redirect()->route('estadocivil.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        if($this->m_canombr_campos=="llave_compuesta"){
            $estadosciviles =  Capmesci::nombre($this->m_canombr_filtro)->
                            ...
                            orderBy(' Capmesci.m_canombr',$this->m_canombr_ordene)->get();
        }else{
            $estadosciviles =  Capmesci::nombre($this->m_canombr_filtro)->
                            Codigo_nacional($this->m_cacodna_filtro)->
                            estado($this->m_caestad_filtro)->
        ...
        $vista = \view('admin.programador.estadocivil.exportarpdf',compact('estadosciviles'))->render();
        ...
        $nombre_archivo = 'estadosciviles_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'estadosciviles_'.$cadena_fecha_actual;

        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('estadosciviles', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA ESTADO CIVIL';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $estadosciviles->Count();
                ...
                foreach($estadosciviles as $index => $estadocivil) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $estadocivil->m_nucodig, $estadocivil->m_canombr,
                            $estadocivil->m_cacodna, $estadocivil->m_caestad,
                            $estadocivil->created_at, $estadocivil->usuario_registra->name,
                            $estadocivil->updated_at, $estadocivil->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $estadocivil->m_nucodig, $estadocivil->m_canombr,
                ...
                //$vista = \view('admin.programador.estadosscivil.exportarexcel',compact('estadosciviles'))->render();
                //$sheet->fromArray($estadosciviles);

...
//Nombre:           DocumentosControler.php
//Descripción:      Documentos para control por fecha de vencimiento (humanos - robots - vehículos)
//Actualizaciones
...
class DocumentosController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $documentos = Capmdocu::nombre($request->m_canombr)->
            ...
            //  VISTA
            return view('admin.programador.documentos.index',
                compact('documentos','usuarios','m_canombr_filtro',
    ...
    public function create(){
        return view('admin.programador.documentos.crear');
    }
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'documentos';

        //2- CONSULTA SI EL NOMBRE YA EXISTE
        //$documentos = Capmdocu::where('m_canombr',$request->m_canombr)->get();
        $documentos = Capmdocu::where('m_canombr',$request->m_canombr)->get();
        ...
        if(count($documentos)>0){
            ...
            //return redirect()->route('documentos.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            ...
            return redirect()->route('documentos.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
        $documento = Capmdocu::find($id);
        $causreg = User::find($documento->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
        $documento['nombre_causreg'] = $nombre_usuario_causreg;
    ...
    public function show($id){
        $documento = Capmdocu::find($id);
        //return response()->json($documento);
        return view('admin.programador.documentos.consultar',compact('documento'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmdocu::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if($documento->m_canombr==$request->m_canombr){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $documento = Capmdocu::find($id);
            $documento->m_canombr = $request->m_canombr;
            $documento->m_cacodna = $request->m_cacodna;
            $documento->m_nuorden = $request->m_nuorden;
            $documento->m_caresol = $request->m_caresol;
            $documento->m_feresol = $request->m_feresol;
            $documento->m_cadescr = $request->m_cadescr;
            $documento->m_caapper = $request->m_caapper;
            $documento->m_caapvpa = $request->m_caapvpa;
            $documento->m_caapvca = $request->m_caapvca;
            $documento->m_caaprem = $request->m_caaprem;
            $documento->m_cacoven = $request->m_cacoven;
            $documento->m_nudiapv = $request->m_nudiapv;
            $documento->m_caacmav = $request->m_caacmav;
            $documento->m_caapesv = $request->m_caapesv;
            $documento->m_caacrma = $request->m_caacrma;
            $documento->m_caestad = $request->m_caestad;
            $documento->m_causact = Auth::user()->id;
            $documento->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmdocu::find($id);
        ...
        } else if($documento->m_canombr!==$request->m_canombr){
            
            //2-3 EL NOMBRE FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NOMBRE NO ESTÉ YA EN LA TABLA
            $validar = Capmdocu::where('m_canombr',$request->m_canombr)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($documento){
            ...
            $tit_men_res_cor = 'Documento: ';
            $tx1_men_res_cor = 'Nombre: '.$documento->m_canombr.' ';
            $tx2_men_res_cor = '';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            return redirect()->route('documentos.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        if($this->m_canombr_campos=="llave_compuesta"){
            $documentos = Capmdocu::nombre($this->m_canombr_filtro)->
        ...
        $vista = \view('admin.programador.documentos.exportarpdf',compact('documentos'))->render();
        ...
        $nombre_archivo = 'documentos_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'documentos_'.$cadena_fecha_actual;
        ...
        Excel::create($nombre_archivo, function($excel) {
            ...
            if($this->m_canombr_campos=="llave_compuesta"){
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA DOCUMENTOS';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $documentos->Count();
                ...
                foreach($documentos as $index => $documento) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $documento->m_nucodig, $documento->m_canombr, $documento->m_cadescr,
                            $documento->m_caapper, $documento->m_caapvpa,
                            $documento->m_caapvca, $documento->m_caaprem,
                            $documento->m_caresol, $documento->m_feresol,
                            $documento->m_cacoven, $documento->m_nudiapv,
                            $documento->m_caacmav, $documento->m_caapesv,
                            $documento->m_caacrma,
                            $documento->m_cacodna, $documento->m_caestad,
                            $documento->created_at, $documento->usuario_registra->name,
                            $documento->updated_at, $documento->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $documento->m_nucodig, $documento->m_canombr, $documento->m_cadescr,
                ...
                //$vista = \view('admin.programador.documentos.exportarexcel',compact('documentos'))->render();

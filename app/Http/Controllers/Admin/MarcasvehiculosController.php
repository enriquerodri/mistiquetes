...
//Nombre:           MarcasvehiculosControler.php
//Descripción:      Marcas para clasificación vehículos
//Actualizaciones
...
class MarcasvehiculosController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $marcas = Capmmave::nombre($request->m_canombr)->
            ...
            //  VISTA
            return view('admin.vehiculos.vehiculos.marcas.index',
                compact('marcas','usuarios','m_canombr_filtro',
    ...
    public function create(){
        return view('admin.vehiculos.vehiculos.marcas.crear');
    }
    ...
    public function store(Request $request){
        ...
        //$marcas = Capmmave::where('m_canombr',$request->m_canombr)->get();
        $marcas = Capmmave::where('m_canombr',$request->m_canombr)->get();
        ...
        if(count($marcas)>0){
            ...
            $tit_men_res_err = 'Marca vehiculo: ';
            ...
            //return redirect()->route('marcas.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            ...
            $tit_men_res_cor = 'Marca vehiculo: ';
            ...
            return redirect()->route('marcas.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
        $marca= Capmmave::find($id);
        $causreg = User::find($marca->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
        $marca['nombre_causreg'] = $nombre_usuario_causreg;
    ...
    public function show($id){
        $marca= Capmmave::find($id);
        //return response()->json($marca);
        return view('admin.vehiculos.vehiculos.marcas.consultar',compact('contrato'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmmave::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if($marca->m_canombr==$request->m_canombr){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $marca= Capmmave::find($id);
            $marca->m_canombr = $request->m_canombr;
            $marca->m_cacodna = $request->m_cacodna;
            $marca->m_caestad = $request->m_caestad;
            $marca->m_causact = Auth::user()->id;
            $marca->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmmave::find($id);
    ...
    public function destroy($id){
        ...
        if($marca){
            ...
            $tit_men_res_cor = 'Marca vehiculo: ';
            $tx1_men_res_cor = 'Nombre: '.$marca->m_canombr.' ';
            $tx2_men_res_cor = '';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            return redirect()->route('marcas.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        if($this->m_canombr_campos=="llave_compuesta"){
            $marcas = Capmmave::nombre($this->m_canombr_filtro)->
                            ...
                            orderBy(' Capmmave.m_canombr',$this->m_canombr_ordene)->get();
        }else{
            $marcas = Capmmave::nombre($this->m_canombr_filtro)->
                            Codigo_nacional($this->m_cacodna_filtro)->
                            estado($this->m_caestad_filtro)->
        ...
        $vista = \view('admin.vehiculos.vehiculos.marcas.exportarpdf',compact('marcas'))->render();
        ...
        $nombre_archivo = 'marcas_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'marcas_'.$cadena_fecha_actual;
        ...
        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('marcas', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA MARCAS VEHICULO';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $marcas->Count();
                ...
                foreach($marcas as $index => $marca) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $marca->m_nucodig, $marca->m_canombr,
                            $marca->m_cacodna, $marca->m_caestad,
                            $marca->created_at, $marca->usuario_registra->name,
                            $marca->updated_at, $marca->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $marca->m_nucodig, $marca->m_canombr,

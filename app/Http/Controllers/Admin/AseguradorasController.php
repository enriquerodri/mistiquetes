...
//Nombre:           AseguradorasControler.php
...
class AseguradorasController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            //  ASEGURADORAS
            $aseguradoras = Capmaseg::Nuipe($request->m_canuipe)->
            ...
            foreach($aseguradoras as $aseguradora){
                $ciudad = Capmciud::where('m_nucodig',$aseguradora->m_nucociu)->first();
            ...
            //  VISTA
            return view('admin.programador.aseguradoras.index',
                compact('aseguradoras','tiposdocumentos','ciudades','departamentos',
    ...
    public function create(){
        ...
        $usuarios = User::orderBy('name','asc')->pluck('name','id');
        //  VISTA
        return view('admin.programador.aseguradoras.crear',
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'Compañias aseguradoras';

        //2- CONSULTA SI EL NUIP YA EXISTE
        $aseguradoras = Capmaseg::where('m_canuipe',$request->m_canuipe)->get();
        if(count($aseguradoras)>0){

            //2-1 EL NUEVO NOMBRE YA ESTA EN LA TABLA
            //2-2 RESULTADO ERROR: MENSAJES
            $tit_men_res_err = 'Compañia aseguradora: ';
            ...
            return redirect()->route('aseguradoras.index',['m_canuipe'=>$request->m_canuipe])->with($par_men_res_err);
        }
        else{
            
            //2-1 EL NOMBRE NUEVO NO EXISTE
            //2-2 INSERTAR LOS DATOS
            $aseguradora = Capmaseg::create([
            ...
            $tit_men_res_cor = 'Compañia aseguradora: ';
            ...
            return redirect()->route('aseguradoras.index')->with($par_men_res_cor);
        }
    }
    ...
    public function show($id){
        ...
        $ciudades = Capmciud::orderBy('m_canombr','asc')->pluck('m_canombr','m_nucodig');
        //return response()->json($aseguradora);
        return view('admin.programador.aseguradoras.consultar',compact('aseguradora','tiposdocumentos','ciudades'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmaseg::find($id);

        //2-2 COMPARA NUIP (LLAVE) REGISTRO HALLADO CON NUIP (LLAVE) DE LA RESPUESTA
        if($aseguradora->m_canuipe==$request->m_canuipe){

            //2-3 CORRESPONDE AL MISMO NUIP (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $aseguradora = Capmaseg::find($id);
            $aseguradora->m_canuipe = $request->m_canuipe;
            $aseguradora->m_nudiver = $request->m_nudiver;
            $aseguradora->m_nutipdo = $request->m_nutipdo;
            $aseguradora->m_canombr = $request->m_canombr;
            $aseguradora->m_cadirec = $request->m_cadirec;
            $aseguradora->m_nucociu = $request->m_nucociu;
            $aseguradora->m_catelfi = $request->m_catelfi;
            $aseguradora->m_camovil = $request->m_camovil;
            $aseguradora->m_casitew = $request->m_casitew;
            $aseguradora->m_cacorel = $request->m_cacorel;
            $aseguradora->m_cadacon = $request->m_cadacon;
            $aseguradora->m_caautna = $request->m_caautna;
            $aseguradora->m_cacodna = $request->m_cacodna;
            $aseguradora->m_caestad = $request->m_caestad;
            $aseguradora->m_causact = Auth::user()->id;
            $aseguradora->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmaseg::find($id);
        ...
        } else if($aseguradora->m_canuipe!==$request->m_canuipe) {
            
            //2-3 EL NUIP FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NUIP NO ESTÉ YA EN LA TABLA
            $validar = Capmaseg::where('m_canuipe',$request->m_canuipe)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($aseguradora){
            ...
            $tit_men_res_cor = 'Compañia aseguradora: ';
            $tx1_men_res_cor = ' '.$aseguradora->m_canuipe.' ';
            $tx2_men_res_cor = ' '.$aseguradora->m_canombr.' ';
            $tx3_men_res_cor = ' '.$aseguradora->m_cacodna.' ';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            $audi_borrar = Captaudi::create([
                'm_nuidope'=>$aud_idu_ope,
                'm_canombr'=>$aud_nom_pag,
                'm_callave'=>$aseguradora->m_canuipe,
                'm_nuidusu'=>Auth::user()->id,
                'm_cadeini'=>$aseguradora]);
            ...
            return redirect()->route('aseguradoras.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        $aseguradoras = Capmaseg::Nuipe($this->m_canuipe_filtro)->
        ...
        $vista = \view('admin.programador.aseguradoras.exportarpdf',compact('aseguradoras'))->render();
        $pdf->loadHTML($vista);
        ...
        $nombre_archivo = 'aseguradoras_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'aseguradoras_'.$cadena_fecha_actual;

        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('Aseguradoras', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA DE COMPAÑIAS ASEGURADORAS';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $aseguradoras->Count();
                ...
                $sheet->row(7, [
                        ...
                        'Tipo de documento','Aseguradora', 'Dirección',
                ...
                //13-10-2018
                $j=0;
                foreach($aseguradoras as $index => $aseguradora) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $aseguradora->m_nucodig,
                            $aseguradora->m_canuipe, $aseguradora->m_nudiver,
                            $aseguradora->tiposdocumentos->m_canombr,
                            $aseguradora->m_canombr, $aseguradora->m_cadirec,
                            $aseguradora->ciudades->m_canombr,
                            $departamento_x[$j],
                            $pais_x[$j],
                            $aseguradora->m_catelfi, $aseguradora->m_camovil, $aseguradora->m_casitew,
                            $aseguradora->m_cacorel, $aseguradora->m_cadacon, $aseguradora->m_caautna,
                            $aseguradora->m_cacodna, $aseguradora->m_caestad,
                            $aseguradora->created_at, $aseguradora->usuario_registra->name,
                            $aseguradora->updated_at, $aseguradora->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $aseguradora->m_nucodig,

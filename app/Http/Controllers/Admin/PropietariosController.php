...
//Descripción:      Propietarios vehículos
//Actualizaciones
...
class PropietariosController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            return view('admin.vehiculos.propietarios.index',
    ...
    public function store(Request $request){
        ...
        if(count($propietarios)>0){
            ...
            return redirect()->route('propietarios.index',['m_canuipe'=>$request->m_canuipe])->with($par_men_res_err);
        }
        else{
            ...
            return redirect()->route('propietarios.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        ...
        $ciudades = Capmciud::orderBy('m_canombr','asc')->pluck('m_canombr','m_nucodig');
        $oficina_registro = Capmsucu::orderBy('m_canombr','asc')->pluck('m_canombr','m_nucodig');
        $vehiculospropietarios = DB::select('select * from Capmvehi where m_caidpac = ?', [$id]);

        //2-1 RESULTADOS: ENVIA
        //return response()->json([$propietario,
        //                        $vehiculospropietarios,
        //]);
    ...
    public function destroy($id){
        ...
        if($propietario){
            ...
            return redirect()->route('propietarios.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        $vista = \view('admin.vehiculos.propietarios.exportarpdf',compact('propietarios'))->render();
        $pdf->loadHTML($vista);
        ...
        $nombre_archivo = 'propietarios_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'propietarios_'.$cadena_fecha_actual;

        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('Propietarios', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA DE PROPIETARIOS';

...
//Nombre:           ColoresvehiculosControler.php
//Descripción:      Colores vehiculos
//Actualizaciones
...
class ColoresvehiculosController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $colores = Capmcolo::nombre($request->m_canombr)->
            ...
            //  VISTA
            return view('admin.vehiculos.vehiculos.colores.index',
                compact('colores','usuarios','m_canombr_filtro',
    ...
    public function create(){
        return view('admin.vehiculos.vehiculos.colores.crear');
    }
    ...
    public function store(Request $request){
        ...
        //$colores = Capmcolo::where('m_canombr',$request->m_canombr)->get();
        $colores = Capmcolo::where('m_canombr',$request->m_canombr)->get();
        ...
        if(count($colores)>0){
            ...
            //return redirect()->route('colores.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            ...
            return redirect()->route('colores.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
        $color= Capmcolo::find($id);
        $causreg = User::find($color->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
        $color['nombre_causreg'] = $nombre_usuario_causreg;
    ...
    public function show($id){
        $color= Capmcolo::find($id);
        //return response()->json($color);
        return view('admin.vehiculos.vehiculos.colores.consultar',compact('contrato'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmcolo::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if($color->m_canombr==$request->m_canombr){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $color= Capmcolo::find($id);
            $color->m_canombr = $request->m_canombr;
            $color->m_cacodna = $request->m_cacodna;
            $color->m_caestad = $request->m_caestad;
            $color->m_causact = Auth::user()->id;
            $color->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmcolo::find($id);
    ...
    public function destroy($id){
        ...
        if($color){
            ...
            $tit_men_res_cor = 'Color: ';
            $tx1_men_res_cor = 'Nombre: '.$color->m_canombr.' ';
            $tx2_men_res_cor = '';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            return redirect()->route('colores.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        if($this->m_canombr_campos=="llave_compuesta"){
            $colores = Capmcolo::nombre($this->m_canombr_filtro)->
                            ...
                            orderBy(' Capmcolo.m_canombr',$this->m_canombr_ordene)->get();
        }else{
            $colores = Capmcolo::nombre($this->m_canombr_filtro)->
                            Codigo_nacional($this->m_cacodna_filtro)->
                            estado($this->m_caestad_filtro)->
        ...
        $vista = \view('admin.vehiculos.vehiculos.colores.exportarpdf',compact('colores'))->render();
        ...
        $nombre_archivo = 'colores_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'colores_'.$cadena_fecha_actual;

        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('colores', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA COLORES VEHICULO';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $colores->Count();
                ...
                foreach($colores as $index => $color) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $color->m_nucodig, $color->m_canombr,
                            $color->m_cacodna, $color->m_caestad,
                            $color->created_at, $color->usuario_registra->name,
                            $color->updated_at, $color->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $color->m_nucodig, $color->m_canombr,

...
//Nombre:           ContratosControler.php
//Descripción:      Tipos de contrato laboral (humanos - robots)
//Actualizaciones
...
class ContratosController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $contratos = Capmtico::nombre($request->m_canombr)->
            ...
            //  VISTA
            return view('admin.programador.contratos.index',
                compact('contratos','usuarios','m_canombr_filtro',
    ...
    public function create(){
        return view('admin.programador.contratos.crear');
    }
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'Contratos';

        //2- CONSULTA SI EL NOMBRE YA EXISTE
        //$contratos = Capmtico::where('m_canombr',$request->m_canombr)->get();
        $contratos = Capmtico::where('m_canombr',$request->m_canombr)->get();
        ...
        if(count($contratos)>0){
            ...
            //return redirect()->route('contratos.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            ...
            return redirect()->route('contratos.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
        $contrato = Capmtico::find($id);
        $causreg = User::find($contrato->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
        $contrato['nombre_causreg'] = $nombre_usuario_causreg;
    ...
    public function show($id){
        $contrato = Capmtico::find($id);
        //return response()->json($contrato);
        return view('admin.programador.contratos.consultar',compact('contrato'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmtico::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if($contrato->m_canombr==$request->m_canombr){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $contrato = Capmtico::find($id);
            $contrato->m_canombr = $request->m_canombr;
            $contrato->m_cacodna = $request->m_cacodna;
            $contrato->m_caestad = $request->m_caestad;
            $contrato->m_causact = Auth::user()->id;
            $contrato->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmtico::find($id);
        ...
        } else if($contrato->m_canombr!==$request->m_canombr){
            
            //2-3 EL NOMBRE FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NOMBRE NO ESTÉ YA EN LA TABLA
            $validar = Capmtico::where('m_canombr',$request->m_canombr)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($contrato){
            ...
            $tit_men_res_cor = 'Tipo contrato: ';
            $tx1_men_res_cor = 'Nombre: '.$contrato->m_canombr.' ';
            $tx2_men_res_cor = '';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            return redirect()->route('contratos.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        if($this->m_canombr_campos=="llave_compuesta"){
            $contratos = Capmtico::nombre($this->m_canombr_filtro)->
                            ...
                            orderBy('Capmtico.m_canombr',$this->m_canombr_ordene)->get();
        }else{
            $contratos = Capmtico::nombre($this->m_canombr_filtro)->
                            Codigo_nacional($this->m_cacodna_filtro)->
                            estado($this->m_caestad_filtro)->
        ...
        $vista = \view('admin.programador.contratos.exportarpdf',compact('contratos'))->render();
        ...
        $nombre_archivo = 'contratos_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'contratos_'.$cadena_fecha_actual;
        ...
        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('Contratos', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA TIPOS DE CONTRATOS';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $contratos->Count();
                ...
                foreach($contratos as $index => $contrato) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $contrato->m_nucodig, $contrato->m_canombr,
                            $contrato->m_cacodna, $contrato->m_caestad,
                            $contrato->created_at, $contrato->usuario_registra->name,
                            $contrato->updated_at, $contrato->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $contrato->m_nucodig, $contrato->m_canombr,
                ...
                //$vista = \view('admin.programador.contratos.exportarexcel',compact('contratos'))->render();

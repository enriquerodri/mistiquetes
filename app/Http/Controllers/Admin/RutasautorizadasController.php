...
//Nombre:           RutasautorizadasControler.php
...
class RutasautorizadasController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $rutasautorizadas = Capmrutp::nombre($request->m_canombr)->
            ...
            //  VISTA
            return view('admin.vehiculos.rutas.autorizadas.index',
                compact('rutasautorizadas','usuarios','m_canombr_filtro',
    ...
    public function create(){
        return view('admin.vehiculos.rutas.autorizadas.crear');
    }
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'rutas-autorizadas';

        //2- CONSULTA SI EL NOMBRE YA EXISTE
        //$rutasautorizadas = Capmrutp::where('m_canombr',$request->m_canombr)->get();
        $rutasautorizadas = Capmrutp::where('m_canombr',$request->m_canombr)->get();

        if(count($rutasautorizadas)>0){

            //2-1 EL NUEVO NOMBRE YA ESTA EN LA TABLA
            //2-2 RESULTADO ERROR: MENSAJES
            $tit_men_res_err = 'Ruta autorizada: ';
            ...
            //return redirect()->route('rutas-autorizadas.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            return redirect()->route('rutas-autorizadas.index',
            ...
            $rutaautorizada= Capmrutp::create([
            ...
            $tit_men_res_cor = 'Ruta autorizada: ';
            ...
            return redirect()->route('rutas-autorizadas.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
        $rutaautorizada= Capmrutp::find($id);
        $causreg = User::find($rutaautorizada->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
        $rutaautorizada['nombre_causreg'] = $nombre_usuario_causreg;

        //2-1 RESULTADOS: ENVIA
        return response()->json($rutaautorizada);
    }
    ...
    public function show($id){
        $rutaautorizada= Capmrutp::find($id);
        //return response()->json($rutaautorizada);
        return view('admin.vehiculos.rutas.autorizadas.consultar',compact('contrato'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmrutp::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if($rutaautorizada->m_canombr==$request->m_canombr){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $rutaautorizada= Capmrutp::find($id);
            $rutaautorizada->m_canombr = $request->m_canombr;
            $rutaautorizada->m_cacodna = $request->m_cacodna;
            $rutaautorizada->m_caestad = $request->m_caestad;
            $rutaautorizada->m_causact = Auth::user()->id;
            $rutaautorizada->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmrutp::find($id);
        ...
        } else if($rutaautorizada->m_canombr!==$request->m_canombr){
            
            //2-3 EL NOMBRE FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NOMBRE NO ESTÉ YA EN LA TABLA
            $validar = Capmrutp::where('m_canombr',$request->m_canombr)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($rutaautorizada){
            ...
            $tit_men_res_cor = 'Ruta autorizada: ';
            $tx1_men_res_cor = 'Nombre: '.$rutaautorizada->m_canombr.' ';
            $tx2_men_res_cor = '';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            $audi_borrar = Captaudi::create([
                'm_nuidope'=>$aud_idu_ope,
                'm_canombr'=>$aud_nom_pag,
                'm_callave'=>$rutaautorizada->m_canombr,
                'm_nuidusu'=>Auth::user()->id,
                'm_cadeini'=>$rutaautorizada]);

            //2-4 ELIMINA REGISTRO
            $rutaautorizada->delete();
            ...
            return redirect()->route('rutas-autorizadas.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        if($this->m_canombr_campos=="llave_compuesta"){
            $rutasautorizadas = Capmrutp::nombre($this->m_canombr_filtro)->
                            ...
                            orderBy(' Capmrutp.m_canombr',$this->m_canombr_ordene)->get();
        }else{
            $rutasautorizadas = Capmrutp::nombre($this->m_canombr_filtro)->
                            Codigo_nacional($this->m_cacodna_filtro)->
                            estado($this->m_caestad_filtro)->
        ...
        $vista = \view('admin.vehiculos.rutas.autorizadas.exportarpdf',compact('rutasautorizadas'))->render();
        ...
        $nombre_archivo = 'rutas_autorizadas_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'rutas_autorizadas_'.$cadena_fecha_actual;

        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('rutas_autorizadas', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA RUTAS AUTORIZADAS';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $rutasautorizadas->Count();
                ...
                // REGISTROS
                foreach($rutasautorizadas as $index => $rutaautorizada) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $rutaautorizada->m_nucodig, $rutaautorizada->m_canombr,
                            $rutaautorizada->m_cacodna, $rutaautorizada->m_caestad,
                            $rutaautorizada->created_at, $rutaautorizada->usuario_registra->name,
                            $rutaautorizada->updated_at, $rutaautorizada->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $rutaautorizada->m_nucodig, $rutaautorizada->m_canombr,

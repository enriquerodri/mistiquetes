...
//Nombre:           TiposdocumentosControler.php
//Descripción:      Tipos documento de identificación (humanos)
//Actualizaciones
...
class TiposdocumentosController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->m_capap10=="SI"){
            ...
            if($m_canombr_campos=="llave_compuesta"){
                $tiposdocumentos = Capmtido::nombre($request->m_canombr)->
            ...
            //  VISTA
            return view('admin.programador.tiposdocumentos.index',
                compact('tiposdocumentos','usuarios','m_canombr_filtro',
    ...
    public function create(){
        return view('admin.programador.tiposdocumentos.crear');
    }
    ...
    public function store(Request $request){
        ...
        $aud_idu_ope = 1;
        $aud_nom_pag = 'tiposdocumentos';

        //2- CONSULTA SI EL NOMBRE YA EXISTE
        //$tiposdocumentos = Capmtido::where('m_canombr',$request->m_canombr)->get();
        $tiposdocumentos = Capmtido::where('m_canombr',$request->m_canombr)->get();
        ...
        if(count($tiposdocumentos)>0){
            ...
            $tit_men_res_err = 'Tipo documento: ';
            ...
            //return redirect()->route('tiposdocumentos.index',['m_canombr'=>$request->m_canombr])->with($par_men_res_err);
            ...
            $tit_men_res_cor = 'Tipo documento: ';
            ...
            return redirect()->route('tiposdocumentos.index')->with($par_men_res_cor);
        }
    }
    ...
    public function edit($id){
        //1- PREPARA
        $tipodocumento = Capmtido::find($id);
        $causreg = User::find($tipodocumento->m_causreg);
        $nombre_usuario_causreg = $causreg->name;
        $tipodocumento['nombre_causreg'] = $nombre_usuario_causreg;
    ...
    public function show($id){
        $tipodocumento = Capmtido::find($id);
        //return response()->json($tipodocumento);
        return view('admin.programador.tiposdocumentos.consultar',compact('contrato'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $con_dat_ini = Capmtido::find($id);

        //2-2 COMPARA NOMBRE (LLAVE) REGISTRO HALLADO CON NOMBRE (LLAVE) DE LA RESPUESTA
        if($tipodocumento->m_canombr==$request->m_canombr){

            //2-3 CORRESPONDE AL MISMO NOMBRE (LLAVE) - NO FUE MODIFICADO - ACTUALIZA TODOS LOS CAMPOS
            $tipodocumento = Capmtido::find($id);
            $tipodocumento->m_canombr = $request->m_canombr;
            $tipodocumento->m_cacodna = $request->m_cacodna;
            $tipodocumento->m_caestad = $request->m_caestad;
            $tipodocumento->m_causact = Auth::user()->id;
            $tipodocumento->save();
            
            //2-4 BUSCA EL REGISTRO POR EL ID TOMA VALORES FINALES (AUDITORIA)
            $con_dat_fin = Capmtido::find($id);
        ...
        } else if($tipodocumento->m_canombr!==$request->m_canombr){
            
            //2-3 EL NOMBRE FUE MODIFICADO
            //2-4 CONSULTA PARA REVISAR QUE EL NUEVO NOMBRE NO ESTÉ YA EN LA TABLA
            $validar = Capmtido::where('m_canombr',$request->m_canombr)->get();
            if(count($validar)>0){
    ...
    public function destroy($id){
        ...
        if($tipodocumento){
            ...
            $tit_men_res_cor = 'Tipo documento: ';
            $tx1_men_res_cor = 'Nombre: '.$tipodocumento->m_canombr.' ';
            $tx2_men_res_cor = '';
            $tx3_men_res_cor = '';
            $tx4_men_res_cor = '';
            $tx5_men_res_cor = '';
            ...
            $audi_borrar = Captaudi::create([
                'm_nuidope'=>$aud_idu_ope,
                'm_canombr'=>$aud_nom_pag,
                'm_callave'=>$tipodocumento->m_canombr,
                'm_nuidusu'=>Auth::user()->id,
                'm_cadeini'=>$tipodocumento]);
            ...
            return redirect()->route('tiposdocumentos.index')->with($par_men_res_err);
        }
    }
    ...
    public function exportarpdf(Request $request){
        ...
        if($this->m_canombr_campos=="llave_compuesta"){
            $tiposdocumentos = Capmtido::nombre($this->m_canombr_filtro)->
                            ...
                            orderBy('Capmtido.m_canombr',$this->m_canombr_ordene)->get();
        }else{
            $tiposdocumentos = Capmtido::nombre($this->m_canombr_filtro)->
                            Codigo_nacional($this->m_cacodna_filtro)->
                            estado($this->m_caestad_filtro)->
        ...
        $vista = \view('admin.programador.tiposdocumentos.exportarpdf',compact('tiposdocumentos'))->render();
        ...
        $nombre_archivo = 'tiposdocumentos_'.$cadena_fecha_actual.'.pdf';
        //  Resultados
    ...
    public function exportarexcel(Request $request){
        ...
        $cadena_fecha_actual = $fecha_actual->format("d/m/Y/h/i/s/a");
        // Nombre archivo exportacion
        $nombre_archivo = 'tiposdocumentos_'.$cadena_fecha_actual;

        Excel::create($nombre_archivo, function($excel) {
            $excel->sheet('tiposdocumentos', function($sheet) {
                ...
                // 6- TITULO CONSULTA
                $tit_con_sul = 'CONSULTA TIPOS DE tiposdocumentos';

                // 20- TOTAL REGISTROS
                $tot_reg_ist = $tiposdocumentos->Count();
                ...
                // REGISTROS
                foreach($tiposdocumentos as $index => $tipodocumento) {
                    ...
                    if ($index) {
                            $sheet->row($index+8, [
                            $tipodocumento->m_nucodig, $tipodocumento->m_canombr,
                            $tipodocumento->m_cacodna, $tipodocumento->m_caestad,
                            $tipodocumento->created_at, $tipodocumento->usuario_registra->name,
                            $tipodocumento->updated_at, $tipodocumento->usuario_actualiza->name,
                        ]);
                    } else {
                        $sheet->row($index+8, [
                            $tipodocumento->m_nucodig, $tipodocumento->m_canombr,
                ...
                //$vista = \view('admin.programador.tiposdocumentos.exportarexcel',compact('tiposdocumentos'))->render();
                //$sheet->fromArray($tiposdocumentos);

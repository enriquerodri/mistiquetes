...
//Nombre:           Capmclgl.php
//Descripción:      Clientes globales organizacón empresarial
//Actualizaciones
...
class Capmclgl extends Model
{
	//Administrador Clientes contado
	protected $table = "capmclgl";
	...
	protected $fillable = ['m_canuipe', 'm_nudiver', 'm_nutipdo', 
                ...
                'm_fenacim', 'm_nuedada', 'm_catipsa', 'm_catisex', 'm_nuofici',
                'm_caviemp', 'm_causreg', 'm_causact'];
    ...
    */
    ...
    //Buscador de Clientes convenios empresariales por id - LLAVE (BUSQUEDA EXACTA)
    public function scopeNuipe($query,$m_canuipe){
        return $query->where('m_canuipe','LIKE',$m_canuipe);
    }

    //Buscador de Clientes convenios empresariales por razon social
    public function scopeNombre($query,$nombre){
        return $query->where('m_canombr','LIKE','%'.$nombre.'%');
    }

    //Filtro de Clientes convenios empresariales por ciudades (BUSQUEDA EXACTA)
    public function scopeNombre_ciudad($query,$m_nucociu){
        return $query->where('m_nucociu','LIKE',$m_nucociu);
    }

    //Buscador de Clientes convenios empresariales por nombre contacto

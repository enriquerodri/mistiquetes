...
class Capmanul extends Model
{
	//Administrador Motivos anulaciones
	protected $table = "capmanul";
	protected $primaryKey = 'm_nucodig';
	protected $fillable = ['m_canombr','m_cacodna','m_caaplic','m_caestad',
							'm_caanure','m_causreg','m_causact'];
    ...
    //Buscador de Anulaciones por nombre - LLAVE
    ...
    //Filtro de Anulaciones aplica a - LLAVE
    public function scopeAplicaa($query,$m_caaplic){
        return $query->where('m_caaplic','LIKE','%'.$m_caaplic.'%');
    }

    //Filtro de Anulaciones anular documentos relacionados - LLAVE
    public function scopeAnula_relacionados($query,$m_caanure){
        return $query->where('m_caanure','LIKE','%'.$m_caanure.'%');
    }
